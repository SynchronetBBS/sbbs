...
p = syncretro_parse_title("Astrosmash (1981) (Mattel).int");
...
p = syncretro_parse_title("Atlantis (1981) (Imagic) [!].int");
...
p = syncretro_parse_title("Adventure (AD&D - Cloudy Mountain) (1982) (Mattel).int");
...
p = syncretro_parse_title("Advanced Dungeons & Dragons (1982) (Mattel).int");
eq(p.title, "Advanced Dungeons & Dragons", "ampersand survives");

// Parenthetical that is not a year stays in the title.
p = syncretro_parse_title("Armor Battle (USA, Europe) (Fast Tanks).int");
...
p = syncretro_parse_title("4tris.int");
...
eq(syncretro_parse_title("Utopia (1981) (Mattel).INT").title, "Utopia", "uppercase extension");
eq(syncretro_parse_title("Some.Game.Name.bin").title, "Some.Game.Name", "only last extension");

// Stacked dump markers, as the real set carries them.
p = syncretro_parse_title("League of Light (Prototype) (1983) (Activision) [a1][!].int");
...
p = syncretro_parse_title("Bump 'N' Jump (1982-83) (Mattel).int");
...
p = syncretro_parse_title("River Raid (1982-83) (Activision) [!].int");
...
check(syncretro_quote_safe("Astrosmash (1981) (Mattel).int"), "ordinary name is safe");
check(syncretro_quote_safe("Advanced Dungeons & Dragons (1982) (Mattel).int"), "ampersand is safe");
check(!syncretro_quote_safe('He said "hi".int'), "double quote is unsafe");
check(!syncretro_quote_safe("cost $5.int"), "dollar is unsafe");
check(!syncretro_quote_safe("back\\slash.int"), "backslash is unsafe");
...
var md_a = syncretro_file_md5(dir + "Astrosmash (1981) (Mattel).int", 4096);
var md_b = syncretro_file_md5(dir + "astrosmash.int", 4096);
...
eq(syncretro_file_md5(dir + "does-not-exist.int", 4096), "", "missing file yields empty md5");

writeln("4. discovery");
// The Intellivision's rules, exactly as xtrn/syncivision/lobby.js declares them.
var INTV = syncretro_rules({
	ext:        ["int", "bin", "rom"],
	min_size:   2 * 1024,
	max_size:   64 * 1024,
...
var roms = syncretro_discover(dir, INTV);
...
var pac_roms = syncretro_discover(dir, INTV);
...
var fewer = syncretro_discover(dir, syncretro_rules({ ext: ["int", "bin", "rom"], min_size: 2048,
...
if (file_exists(bios_src)) {
	...
	var guarded = syncretro_discover(dir, INTV);
...
put("grom.bin", 2048, "x");
var by_name = syncretro_discover(dir, INTV);
...
var pdir = "/tmp/syncretro_data/";
if (file_isdir(pdir + "syncretro/"))
	directory(pdir + "syncretro/*").forEach(function (p) { file_remove(p); });
mkpath(pdir);

var ppath = syncretro_plays_path(pdir);
check(ppath.indexOf("syncretro") >= 0, "plays path is under syncretro/");
check(file_isdir(pdir + "syncretro/"), "syncretro_plays_path created the directory");

check(syncretro_log_play(ppath, {t: 100, user: 1, alias: "Digital Man",
                          rom: "Utopia (1981) (Mattel).int", secs: 600}), "log 1");
check(syncretro_log_play(ppath, {t: 200, user: 2, alias: "Nelgin",
                          rom: "Astrosmash (1981) (Mattel).int", secs: 60}), "log 2");
check(syncretro_log_play(ppath, {t: 300, user: 1, alias: "Digital Man",
                          rom: "Astrosmash (1981) (Mattel).int", secs: 30}), "log 3");

var plays = syncretro_read_plays(ppath);
...
junk.close();
eq(syncretro_read_plays(ppath).length, 3, "malformed line skipped, not fatal");

var top = syncretro_top_played(plays, 5);
...
eq(syncretro_top_played(plays, 1).length, 1, "n caps the list");
eq(syncretro_top_played([], 5).length, 0, "no plays, no board");

var last = syncretro_last_play(plays, 1);
check(last !== null, "user 1 has a last play");
eq(last.t, 300, "last play is the most recent, not the first");
eq(syncretro_last_play(plays, 99), null, "unknown user has none");
...
var t1 = syncretro_top_played(tied, 5);
var t2 = syncretro_top_played(tied, 5);
...
eq(syncretro_last_play(same_t, 1).rom, "First.int", "equal timestamps: first record wins");

// system.data_dir carries a trailing slash, but a caller may not.
var noslash = "/tmp/syncretro_slash";
var withslash = "/tmp/syncretro_slash/";
eq(syncretro_plays_path(noslash), syncretro_plays_path(withslash),
   "data_dir with and without a trailing slash agree");
check(file_isdir("/tmp/syncretro_slash/syncretro/"), "syncretro_plays_path created the directory");
directory("/tmp/syncretro_slash/syncretro/*").forEach(function (p) { file_remove(p); });

directory(pdir + "syncretro/*").forEach(function (p) { file_remove(p); });

writeln("7. dump-quality variant collapsing");

eq(syncretro_dump_rank("x [!].int"), 0, "verified marker ranks 0");
eq(syncretro_dump_rank("x.int"), 1, "no marker ranks 1");
eq(syncretro_dump_rank("x [a1].int"), 2, "alternate marker ranks 2");
eq(syncretro_dump_rank("x [a1][!].int"), 0, "stacked markers: best one wins");
eq(syncretro_dump_rank("x [o1].int"), 3, "overdump marker ranks 3");
eq(syncretro_dump_rank("x [b1].int"), 4, "bad-dump marker ranks 4");
eq(syncretro_dump_rank("x [z9].int"), 2, "unrecognized marker ranks 2");

function rom(name)
{
	var p = syncretro_parse_title(name);
...
var lc = syncretro_collapse_variants(league);
...
// syncretro_discover() happened to see first.
var happy = [
	rom("Happy Trails (1983) (Activision).int"),
	rom("Happy Trails (1983) (Activision) [o1].int")
];
var hc = syncretro_collapse_variants(happy);
...
var pc = syncretro_collapse_variants(pacman);
...
var vc = syncretro_collapse_variants(tied_variants);
...
eq(syncretro_columns(79, 36), 1, "below 80 cols: one column, always");
eq(syncretro_columns(40, 36), 1, "narrow terminal: one column");
eq(syncretro_columns(80, 36), 2, "80 cols: two columns");
eq(syncretro_columns(132, 36), 3, "132 cols: three columns");
check(syncretro_columns(80, 200) >= 1, "an absurd cell width still yields one column");

eq(syncretro_page_rows(24, 5, 2), 17, "24 rows minus header and footer");
check(syncretro_page_rows(4, 5, 2) >= 1, "a tiny terminal still shows one row");

var pages = syncretro_paginate([1, 2, 3, 4, 5], 2);
eq(pages.length, 3, "five items, two per page, three pages");
eq(pages[2].length, 1, "last page holds the remainder");
eq(syncretro_paginate([], 10).length, 0, "no items, no pages");

// syncretro_cell now carries \1x attribute codes; measure the VISIBLE width by stripping them.
function vislen(s) { return s.replace(/\x01./g, "").length; }

var cell = syncretro_cell(7, {title: "Astrosmash", year: 1981}, 36);
...
var longcell = syncretro_cell(199, {title: "Advanced Dungeons and Dragons Treasure of Tarmin",
                             year: 1982}, 30);
...
eq(syncretro_platform("Win32"), "win32", "Win32 -> win32");
eq(syncretro_platform("Win64"), "win32", "Win64 host still maps to the Win32 door dir");
// macOS: system.platform ("macOS"/"MacOSX") and deploy's uname ("Darwin")
// must agree on one token.
eq(syncretro_platform("macOS"),  "darwin", "macOS -> darwin");
eq(syncretro_platform("MacOSX"), "darwin", "MacOSX -> darwin");
eq(syncretro_platform("Darwin"), "darwin", "Darwin -> darwin");
// *nixes: lower-cased, matching `uname -s | tr A-Z a-z`.
eq(syncretro_platform("Linux"),   "linux",   "Linux -> linux");
eq(syncretro_platform("FreeBSD"), "freebsd", "FreeBSD -> freebsd");
eq(syncretro_platform("NetBSD"),  "netbsd",  "NetBSD -> netbsd");
// A "/" (as in "GNU/Hurd") must not nest a path.
check(syncretro_platform("GNU/Hurd").indexOf("/") < 0, "no path separator survives in the token");
// The core extension follows from the platform token.
eq(syncretro_core_ext("win32"),  "dll",   "win32 core is .dll");
eq(syncretro_core_ext("darwin"), "dylib", "darwin core is .dylib");
eq(syncretro_core_ext("linux"),  "so",    "linux core is .so");

writeln("N. architecture bucket (system.architecture <-> uname -m must agree)");
// system.architecture spellings...
eq(syncretro_arch("x64"),   "x64",   "system.architecture x64 -> x64");
eq(syncretro_arch("i686"),  "x86",   "system.architecture i686 -> x86");
eq(syncretro_arch("i386"),  "x86",   "system.architecture i386 -> x86");
eq(syncretro_arch("arm64"), "arm64", "system.architecture arm64 -> arm64");
eq(syncretro_arch("armv7"), "arm",   "system.architecture armv7 -> arm");
eq(syncretro_arch("armv8"), "arm64", "system.architecture armv8 (aarch64) -> arm64");
// ...and the `uname -m` spellings for the SAME hosts land on the same bucket.
eq(syncretro_arch("x86_64"),  "x64",   "uname x86_64 -> x64");
eq(syncretro_arch("amd64"),   "x64",   "uname amd64 -> x64");
eq(syncretro_arch("aarch64"), "arm64", "uname aarch64 -> arm64");
eq(syncretro_arch("armv7l"),  "arm",   "uname armv7l -> arm");
// An unrecognized arch passes through (lower-cased, sanitized).
eq(syncretro_arch("riscv64"), "riscv64", "riscv64 passes through");

writeln("N. target sub-dir (platform + arch)");
eq(syncretro_target("Win32", "i686"),   "",            "Windows is flat (.exe/.dll never collide)");
eq(syncretro_target("Win64", "x64"),    "",            "64-bit Windows host is flat too");
eq(syncretro_target("Linux", "x64"),    "linux-x64",   "Linux x86_64");
eq(syncretro_target("Linux", "aarch64"),"linux-arm64", "Linux arm64 does not collide with linux-x64");
eq(syncretro_target("FreeBSD","amd64"), "freebsd-x64", "FreeBSD amd64");
eq(syncretro_target("macOS", "arm64"),  "darwin-arm64","Apple Silicon");
...
eq(syncretro_list(["a", "b"]).join(","), "a,b", "a list from an array");
eq(syncretro_list(" a , b ").join(","),  "a,b", "a list from a string, trimmed");
eq(syncretro_list(null).length, 0, "a missing list is empty, not a crash");

// Sizes: bare bytes, k and m suffixes.
eq(syncretro_size("8192", 0), 8192,       "bare byte count");
eq(syncretro_size("64k", 0),  65536,      "k suffix");
eq(syncretro_size("4m", 0),   4194304,    "m suffix");
eq(syncretro_size("4M", 0),   4194304,    "suffix is case-insensitive");
eq(syncretro_size("", 99),    99,         "an empty size takes the default");
eq(syncretro_size("banana", 99), 99,      "an unparseable size takes the default, NOT 0");
...
var ci = syncretro_console({ name: "Nintendo Entertainment System", short: "NES", profile: "pad" });
...
eq(syncretro_console({ short: "PC Engine" }).id, "pcengine", "spaces do not survive into the id");
eq(syncretro_console({ short: "../etc" }).id, "etc", "a path traversal cannot come through the id");
eq(syncretro_console({ name: "Intellivision" }).short, "Intellivision", "short falls back to name");
eq(syncretro_console({ short: "Intv" }).id, "intv", "the Intellivision id stays intv: saves must not be orphaned");
eq(syncretro_console({ name: "X" }).profile, "pad", "the default profile is the generic RetroPad");
eq(syncretro_console(null).id, "", "no [console] section at all is legal");
...
// read + close per ROM across the wire. syncretro_file_md5() is the ONLY thing in the
...
var croms = "/tmp/syncretro_cache_roms/";
var cdir  = "/tmp/syncretro_cache/";
...
var cpath = syncretro_cache_path(cdir, "intv");
...
var c1 = syncretro_cache_open(cpath);
var r1 = syncretro_discover(croms, INTV, c1);
...
check(syncretro_cache_flush(c1), "cold run writes the cache");
check(file_exists(cpath), "the cache file exists after a flush");

// Warm: same bytes, same mtimes -> ZERO opens, and the same answer.
var c2 = syncretro_cache_open(cpath);
var r2 = syncretro_discover(croms, INTV, c2);
...
var c4 = syncretro_cache_open(cpath);
syncretro_discover(croms, INTV, c4);
syncretro_cache_flush(c4);
var c5 = syncretro_cache_open(cpath);
...
var r7 = syncretro_discover(croms, INTV);
eq(r7.length, r6.length, "syncretro_discover() with no cache argument behaves as before");
