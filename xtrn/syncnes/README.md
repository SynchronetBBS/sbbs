...
an NES is `syncretro.ini` — it names the console, its core, its key bindings and
what a cartridge looks like, and both halves of the door read it.
...
installer offers (prompted) to download the **FCEUmm** core, which is GPLv2 and
freely redistributable, from the libretro buildbot. Decline it if you have no
outbound internet and drop the core in by hand; you can also run it later:
...
- **`[lobby] enter_sound`** — a one-shot sound played when a player enters the
  lobby, on a SyncTERM that can decode audio files (silent on any other
  terminal). A WAV/OGG/FLAC/VOC file you supply, or a wildcard to pick from at
  random. Nothing ships: unlike SyncDOOM, where you can pull a sound out of your
  own WAD, a cartridge has nothing to extract — a console's audio is generated by
