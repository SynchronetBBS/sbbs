...
function load_config(persona_code)
{
    ...
    /* Pull in any installation-specific intent routers (once) now that we
     * have the config naming the module.  No-op if intent_router is unset. */
    ensure_intent_routers(cfg);
...
/* Pluggable deterministic intent routers.  classify_intent() itself ships
 * NO routing -- it runs the registered routers in ascending-priority order
 * and returns the first match.  Routing lives in modules named by
 * chat_llm.ini's `intent_routers` key, loaded once by ensure_intent_routers():
 * stock routing (bbs_directory, this_bbs, external_archives, relay_message) in
 * exec/chat_llm_routes.js, plus any site-local module (e.g. Synchronet's
 * GitLab git_issues/git_commits routing on Vertrauen).  Each router is
 * fn(s) -> {tool, args} | null; lower priority runs first, so a site router
 * can slot between stock ones. */
var INTENT_ROUTERS = [];
function register_intent_router(fn, priority) {
    if (typeof fn == 'function')
        INTENT_ROUTERS.push({ fn: fn, priority: (typeof priority == 'number') ? priority : 50 });
}
var _intent_routers_loaded = false;
function ensure_intent_routers(cfg) {
    if (_intent_routers_loaded) return;
    _intent_routers_loaded = true;     /* mark first -- a bad module shouldn't retry every turn */
    var list = cfg && cfg.intent_routers;
    if (!list) return;
    var mods = String(list).split(/[\s,]+/);
    for (var i = 0; i < mods.length; i++) {
        var mod = mods[i];
        if (!mod) continue;
        if (mod.indexOf('.js') < 0) mod += '.js';
        try {
            load(mod);                 /* runs in engine scope; module calls register_intent_router() */
        } catch (e) {
            log(LOG_WARNING, 'chat_llm: failed to load intent router "' + mod + '": ' + e);
        }
    }
}
...
function classify_intent(input) {
    ...
    if (!s) return null;
    /* Engine ships no routing of its own -- run registered routers (see
     * ensure_intent_routers) in ascending priority; first match wins. */
    var routers = INTENT_ROUTERS.slice().sort(function (a, b) { return a.priority - b.priority; });
    for (var i = 0; i < routers.length; i++) {
        var r = routers[i].fn(s);
        if (r) return r;
    }
    return null;
}
