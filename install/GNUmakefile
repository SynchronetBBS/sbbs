...
ifeq ($(INSTALL),UNIX)
	...
	$(INSBIN) $(SBBSDIR)/src/sbbs3/scfg/$(CCPRE).$(os).lib.$(SUFFIX)/libsbbs.so $(SBBSDIR)/exec/libsbbs.so
	$(INSBIN) $(SBBSDIR)/src/sbbs3/scfg/$(CCPRE).$(os).lib.$(SUFFIX)/libftpsrvr.so $(SBBSDIR)/exec/libftpsrvr.so
