...
The response is a `CSI = Ps n` audio state report with the feature ID
(100 for libsndfile) and availability (0 / 1) as the pair.  Example
responses:
...
confirm.  Raw headerless PCM is not inferred; use an audio container
format with enough metadata for libsndfile to decode it.
