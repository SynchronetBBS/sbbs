...
clears translated and ungotten keyboard input, resets physical-key state,
and invalidates queued or partially assembled mouse gestures.  Mouse events
are generation-tagged so backend mouse threads cannot publish an old event
after the reset returns.
