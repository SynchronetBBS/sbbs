    ...
    const uint8_t *data, size_t data_len, struct cbdata *cbd)
{
	...
	try {
		...
		size_t pos = 0;
		int    sret = dssh_serialize_uint32(RSA_SHA2_512_NAME_LEN,
		    buf, needed, &pos);
		if (sret < 0) {
			free(buf);
			return sret;
...
extern "C" int
dssh_botan_rsa512_sign(uint8_t **out, size_t *outlen,
...
extern "C" int
dssh_botan_rsa512_verify(const uint8_t *key_blob, size_t key_blob_len,
...
DSSH_PUBLIC int
dssh_rsa_sha2_512_load_key_file(const char *path, dssh_pem_password_cb pw_cb,
    void *pw_cbdata)
{
	if (path == NULL)
		return DSSH_ERROR_INIT;
...
DSSH_PUBLIC int
dssh_rsa_sha2_512_save_key_file(const char *path, dssh_pem_password_cb pw_cb,
    void *pw_cbdata)
{
	if (path == NULL)
		return DSSH_ERROR_INIT;
...
dssh_rsa_sha2_512_save_pub_file(const char *path)
{
	...
	if (str == NULL)
		return DSSH_ERROR_ALLOC;
	int64_t res = dssh_rsa_sha2_512_get_pub_str(str,
