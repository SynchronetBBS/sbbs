...
#define dssh_parse(buf, bufsz, val) _Generic(val,                \
	dssh_byte : dssh_parse_byte(buf, bufsz, val),      \
	dssh_bytearray : dssh_parse_bytearray(buf, bufsz, val),      \
	dssh_boolean : dssh_parse_boolean(buf, bufsz, val), \
	dssh_uint32_t : dssh_parse_uint32(buf, bufsz, val),    \
	dssh_uint64_t : dssh_parse_uint64(buf, bufsz, val),     \
	dssh_string : dssh_parse_string(buf, bufsz, val),      \
	dssh_mpint : dssh_parse_mpint(buf, bufsz, val),         \
	dssh_namelist : dssh_parse_namelist(buf, bufsz, val))

#define dssh_serialized_length(val) _Generic(val,                \
	dssh_byte : dssh_serialized_byte_length(val),      \
	dssh_bytearray : dssh_serialized_byte_length(val),      \
	dssh_boolean : dssh_serialized_boolean_length(val), \
	dssh_uint32_t : dssh_serialized_uint32_length(val),    \
	dssh_uint64_t : dssh_serialized_uint64_length(val),     \
	dssh_string : dssh_serialized_string_length(val),      \
	dssh_mpint : dssh_serialized_mpint_length(val),         \
	dssh_namelist : dssh_serialized_namelist_length(val))

#define dssh_serialize(val, buf, bufsz, pos) _Generic(val,                \
	dssh_byte : dssh_serialize_byte(val, buf, bufsz, pos),      \
	dssh_bytearray : dssh_serialize_byte(val, buf, bufsz, pos),      \
	dssh_boolean : dssh_serialize_boolean(val, buf, bufsz, pos), \
	dssh_uint32_t : dssh_serialize_uint32(val, buf, bufsz, pos),    \
	dssh_uint64_t : dssh_serialize_uint64(val, buf, bufsz, pos),     \
	dssh_string : dssh_serialize_string(val, buf, bufsz, pos),      \
	dssh_mpint : dssh_serialize_mpint(val, buf, bufsz, pos),         \
	dssh_namelist : dssh_serialize_namelist(val, buf, bufsz, pos))
