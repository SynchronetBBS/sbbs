...
**CONFORMS** — `dssh_transport_version_exchange()` calls
...
**CONFORMS** — `dssh_session_init()` accepts a `max_packet_size`
...
naturally wraps at 2^32.  `dssh_transport_rekey()` does NOT reset
...
**CONFORMS** — `dssh_transport_kexinit()` at lines 817–833 handles
...
`dssh_session_init()` without registering algorithms is a
programming error.

### 7.1-5
> The 'cookie' **MUST** be a random value generated by the sender.

**CONFORMS** — `dssh_transport_kexinit()` at line 651 uses
...
**CONFORMS** — `dssh_transport_kexinit()` at lines 800–812 checks
...
**CONFORMS** — After KEXINIT negotiation, `dssh_transport_kex()`
...
(first key exchange).  `dssh_transport_rekey()` does not modify
`session_id`.
...
`dssh_transport_rekey()`, which sends our KEXINIT, runs KEX, and
...
and never modified.  `dssh_transport_rekey()` preserves the
client/server role.
...
`dssh_transport_rekey_needed()` checks all three thresholds and
...
**CONFORMS** — `dssh_transport_send_unimplemented()` sends
