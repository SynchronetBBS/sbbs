...
parse_mp_mpint(const uint8_t *buf, size_t bufsz, Botan::BigInt *mp)
{
	...
	*mp = Botan::BigInt::from_bytes(
	    std::span<const uint8_t>(&buf[4], len));
	return 4 + len;
}
...
static int
botan_client_keygen(const uint8_t *group_payload, size_t group_len,
    size_t *consumed,
    ...
    void **dh_ctx)
{
	...
	pv = parse_mp_mpint(group_payload + p_consumed,
	    group_len - p_consumed, &g);
	if (pv < 0)
		return static_cast<int>(pv);
#if SIZE_MAX < INT64_MAX
	if (pv > SIZE_MAX)
	...
	*consumed = p_consumed + g_consumed;
	...
	Botan::BigInt x = Botan::BigInt::random_integer(
	    Botan::system_rng(),
	    Botan::BigInt::power_of_2(x_bits - 1),
...
static int
botan_client_derive(void *dh_ctx,
...
static int
botan_server_keygen(const uint8_t *p_bytes, size_t p_len,
    ...
    void **dh_ctx)
{
	Botan::BigInt p = Botan::BigInt::from_bytes(
	    std::span<const uint8_t>(p_bytes, p_len));
	Botan::BigInt g = Botan::BigInt::from_bytes(
	    std::span<const uint8_t>(g_bytes, g_len));
	...
	Botan::BigInt y = Botan::BigInt::random_integer(
	    Botan::system_rng(),
	    Botan::BigInt::power_of_2(y_bits - 1),
...
static int
botan_server_derive(void *dh_ctx,
