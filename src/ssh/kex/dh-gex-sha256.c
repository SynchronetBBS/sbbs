    ...
    uint8_t *hash_out)
{
	...
	{ size_t tp = 0; dssh_serialize_uint32((uint32_t)v_s_len, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	ok = ok && EVP_DigestUpdate(mdctx, v_s, v_s_len);

	{ size_t tp = 0; dssh_serialize_uint32((uint32_t)i_c_len, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	ok = ok && EVP_DigestUpdate(mdctx, i_c, i_c_len);

	{ size_t tp = 0; dssh_serialize_uint32((uint32_t)i_s_len, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	ok = ok && EVP_DigestUpdate(mdctx, i_s, i_s_len);

	{ size_t tp = 0; dssh_serialize_uint32((uint32_t)k_s_len, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	ok = ok && EVP_DigestUpdate(mdctx, k_s, k_s_len);

	{ size_t tp = 0; dssh_serialize_uint32(gex_min, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	{ size_t tp = 0; dssh_serialize_uint32(gex_n, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	{ size_t tp = 0; dssh_serialize_uint32(gex_max, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
...
handler(dssh_session sess)
{
	...
	if (sess->trans.client) {
		...
		{
			size_t rpos = 1;
			if (rpos >= payload_len) return DSSH_ERROR_PARSE;
			int64_t n = parse_bn_mpint(&payload[rpos], payload_len - rpos, &p);
			if (n < 0) return (int)n;
			rpos += n;
			if (rpos >= payload_len) { BN_free(p); return DSSH_ERROR_PARSE; }
		...
		res = dssh_transport_recv_packet(sess, &msg_type, &payload, &payload_len);
		if (res < 0) { BN_clear_free(x); goto cleanup; }
		if (msg_type != SSH_MSG_KEX_DH_GEX_REPLY) { BN_clear_free(x); res = DSSH_ERROR_PARSE; goto cleanup; }
		...
		k_bn = BN_new();
		if (!k_bn || BN_mod_exp(k_bn, f_bn, x, p, bnctx) != 1) { BN_clear_free(x); res = DSSH_ERROR_INIT; goto cleanup; }
		BN_clear_free(x);

		/* Store shared secret */
		int k_bytes = BN_num_bytes(k_bn);
		sess->trans.shared_secret = malloc(k_bytes);
		if (!sess->trans.shared_secret) { res = DSSH_ERROR_ALLOC; goto cleanup; }
		...
		if (!sess->trans.exchange_hash) { res = DSSH_ERROR_ALLOC; goto cleanup; }
		...
		res = dssh_transport_recv_packet(sess, &msg_type, &payload, &payload_len);
		if (res < 0) return res;
		if (msg_type != SSH_MSG_KEX_DH_GEX_REQUEST) return DSSH_ERROR_PARSE;
		if (payload_len < 1 + 12) return DSSH_ERROR_PARSE;
		...
		res = dssh_transport_recv_packet(sess, &msg_type, &payload, &payload_len);
		if (res < 0) goto cleanup;
		if (msg_type != SSH_MSG_KEX_DH_GEX_INIT) { res = DSSH_ERROR_PARSE; goto cleanup; }
		{
			size_t rpos = 1;
		...
		if (!dh_value_valid(e_bn, p)) { res = DSSH_ERROR_INVALID; goto cleanup; }
		...
		/* 7. Send GEX_REPLY(K_S, f, sig) */
		{
			...
			if (!reply) { res = DSSH_ERROR_ALLOC; goto cleanup; }
			size_t pos = 0;
