...
serialize_bn_mpint(const BIGNUM *bn, uint8_t *buf, size_t bufsz, size_t *pos)
{
	...
	deuce_ssh_serialize_uint32(mpint_len, buf, bufsz, pos);
	if (need_pad)
		buf[(*pos)++] = 0;
...
parse_bn_mpint(const uint8_t *buf, size_t bufsz, BIGNUM **bn)
{
	...
	uint32_t len;
	if (deuce_ssh_parse_uint32(buf, bufsz, &len) < 4)
		return DEUCE_SSH_ERROR_PARSE;
	if (4 + len > bufsz)
		return DEUCE_SSH_ERROR_PARSE;
    ...
    uint8_t *hash_out)
{
	...
	{ size_t tp = 0; deuce_ssh_serialize_uint32((uint32_t)v_c_len, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	ok = ok && EVP_DigestUpdate(mdctx, v_c, v_c_len);

	{ size_t tp = 0; deuce_ssh_serialize_uint32((uint32_t)v_s_len, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	ok = ok && EVP_DigestUpdate(mdctx, v_s, v_s_len);

	{ size_t tp = 0; deuce_ssh_serialize_uint32((uint32_t)i_c_len, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	ok = ok && EVP_DigestUpdate(mdctx, i_c, i_c_len);

	{ size_t tp = 0; deuce_ssh_serialize_uint32((uint32_t)i_s_len, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	ok = ok && EVP_DigestUpdate(mdctx, i_s, i_s_len);

	{ size_t tp = 0; deuce_ssh_serialize_uint32((uint32_t)k_s_len, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	ok = ok && EVP_DigestUpdate(mdctx, k_s, k_s_len);

	{ size_t tp = 0; deuce_ssh_serialize_uint32(gex_min, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	{ size_t tp = 0; deuce_ssh_serialize_uint32(gex_n, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);
	{ size_t tp = 0; deuce_ssh_serialize_uint32(gex_max, lenbuf, 4, &tp); }
	ok = ok && EVP_DigestUpdate(mdctx, lenbuf, 4);

	{
		...
		mp = 0; mres = serialize_bn_mpint(p, mpbuf, sizeof(mpbuf), &mp);
		ok = ok && (mres == 0) && EVP_DigestUpdate(mdctx, mpbuf, mp);

		mp = 0; mres = serialize_bn_mpint(g, mpbuf, sizeof(mpbuf), &mp);
		ok = ok && (mres == 0) && EVP_DigestUpdate(mdctx, mpbuf, mp);

		mp = 0; mres = serialize_bn_mpint(e, mpbuf, sizeof(mpbuf), &mp);
		ok = ok && (mres == 0) && EVP_DigestUpdate(mdctx, mpbuf, mp);

		mp = 0; mres = serialize_bn_mpint(f, mpbuf, sizeof(mpbuf), &mp);
		ok = ok && (mres == 0) && EVP_DigestUpdate(mdctx, mpbuf, mp);
...
handler(deuce_ssh_session sess)
{
	...
	if (sess->trans.client) {
		...
		/* 1. Send GEX_REQUEST */
		{
			...
			deuce_ssh_serialize_uint32(client_min, msg, sizeof(msg), &pos);
			deuce_ssh_serialize_uint32(client_n, msg, sizeof(msg), &pos);
			deuce_ssh_serialize_uint32(client_max, msg, sizeof(msg), &pos);
		...
		if (!sess->trans.shared_secret) { res = DEUCE_SSH_ERROR_ALLOC; goto cleanup; }
		...
		if (!ka || !ka->verify) { res = DEUCE_SSH_ERROR_INIT; goto cleanup; }
		res = ka->verify(k_s, ks_len, sig_h, sig_len, hash, SHA256_DIGEST_LEN);
		if (res < 0) goto cleanup;

		sess->trans.exchange_hash = malloc(SHA256_DIGEST_LEN);
		if (!sess->trans.exchange_hash) { res = DEUCE_SSH_ERROR_ALLOC; goto cleanup; }
		...
		res = deuce_ssh_transport_recv_packet(sess, &msg_type, &payload, &payload_len);
		if (res < 0) return res;
		if (msg_type != SSH_MSG_KEX_DH_GEX_REQUEST) return DEUCE_SSH_ERROR_PARSE;
		if (payload_len < 1 + 12) return DEUCE_SSH_ERROR_PARSE;
		deuce_ssh_parse_uint32(&payload[1], payload_len - 1, &client_min);
		deuce_ssh_parse_uint32(&payload[5], payload_len - 5, &client_n);
		deuce_ssh_parse_uint32(&payload[9], payload_len - 9, &client_max);

		/* 2. Select and send a DH group via the application's provider. */
		struct deuce_ssh_dh_gex_provider *prov =
		    (struct deuce_ssh_dh_gex_provider *)sess->trans.kex_ctx;
		...
		free(p_bytes);
		free(g_bytes);
		if (!p || !g) { res = DEUCE_SSH_ERROR_ALLOC; goto cleanup; }

		{
			...
			if (res < 0) goto cleanup;
			res = deuce_ssh_transport_send_packet(sess, group_msg, pos, NULL);
			if (res < 0) goto cleanup;
		}
		...
		/* 7. Send GEX_REPLY(K_S, f, sig) */
		{
			...
			if (!reply) { res = DEUCE_SSH_ERROR_ALLOC; goto cleanup; }
			size_t pos = 0;
			reply[pos++] = SSH_MSG_KEX_DH_GEX_REPLY;
			deuce_ssh_serialize_uint32((uint32_t)k_s_len, reply, reply_sz, &pos);
			memcpy(&reply[pos], k_s_buf, k_s_len);
			pos += k_s_len;
			res = serialize_bn_mpint(f_bn, reply, reply_sz, &pos);
			if (res < 0) { free(reply); goto cleanup; }
			deuce_ssh_serialize_uint32((uint32_t)sig_len, reply, reply_sz, &pos);
			memcpy(&reply[pos], sig_buf, sig_len);
			pos += sig_len;
			res = deuce_ssh_transport_send_packet(sess, reply, pos, NULL);
			free(reply);
			if (res < 0) goto cleanup;
		}
