...
static int
botan_x25519_derive(void *priv_ctx, const uint8_t *peer_pub,
...
static int
botan_x25519_exchange(const uint8_t *peer_pub, size_t peer_pub_len,
...
static int
sntrup_encaps(uint8_t *ct_out, uint8_t *ss_out,
...
static int
sntrup_decaps(uint8_t *ss_out, const uint8_t *ct,
...
{
	try {
		return hybrid_pq_handler_impl(kctx, &sntrup_params,
		    &botan_sntrup_ops);
	} catch (...) {
		return DSSH_ERROR_INIT;
	}
