...
static int
ossl_x25519_derive(void *priv_ctx, const uint8_t *peer_pub,
    uint8_t ss_out[HYBRID_PQ_X25519_KEY_LEN])
{
	...
	peer_key = EVP_PKEY_new_raw_public_key_ex(NULL, "X25519", NULL,
	    peer_pub, HYBRID_PQ_X25519_KEY_LEN);
	if (peer_key == NULL)
		return DSSH_ERROR_INIT;
...
static int
ossl_x25519_exchange(const uint8_t *peer_pub, size_t peer_pub_len,
