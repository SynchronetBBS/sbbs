...
static small F3_freeze(int16_t x) { return (small)(x - 3 * ((10923 * x + 16384) >> 15)); }
...
static int R3_recip(small *out, const small *in) {
  ...
  for (loop = 0; loop < 2 * p - 1; ++loop) {
    ...
    v[0] = 0;
    sign = -g[0] * f[0];
    swap = (int16_t)(crypto_int16_negative_mask((crypto_int16)(-delta)) & crypto_int16_nonzero_mask(g[0]));
...
static int Hash_prefix(unsigned char *out, int b, const unsigned char *in, int inlen) {
  ...
  if (crypto_hash_sha512(h, x, (unsigned long long)(inlen + 1)) < 0) return -1;
  for (i = 0; i < 32; ++i) out[i] = h[i];
  return 0;
}


static int Short_random(small *out) {
  uint32_t L[p];
...
static int Small_random(small *out) {
  int i;
  uint32_t L[p];
  if (randombytes(L, sizeof(L)) < 0) return -1;
...
static int KeyGen(Fq *h, small *f, small *ginv) {
  small g[p];
  Fq finv[p];
  for (;;) {
    int result;
    if (Small_random(g) < 0) return -1;
    result = R3_recip(ginv, g);
    crypto_declassify(&result, sizeof result);
    if (result == 0) break;
  }
  if (Short_random(f) < 0) return -1;
  Rq_recip3(finv, f);
...
static int HashConfirm(unsigned char *h, const unsigned char *r, const unsigned char *cache) {
  unsigned char x[Hash_bytes * 2];
  int i;
  if (Hash_prefix(x, 3, r, Small_bytes) < 0) return -1;
  for (i = 0; i < Hash_bytes; ++i) x[Hash_bytes + i] = cache[i];
  return Hash_prefix(h, 2, x, sizeof x);
}

static int HashSession(unsigned char *k, int b, const unsigned char *y, const unsigned char *z) {
  unsigned char x[Hash_bytes + crypto_kem_sntrup761_CIPHERTEXTBYTES];
  int i;
  if (Hash_prefix(x, 3, y, Small_bytes) < 0) return -1;
...
int crypto_kem_sntrup761_keypair(unsigned char *pk, unsigned char *sk) {
  ...
  if (randombytes(sk, Small_bytes) < 0) return -1;
  if (Hash_prefix(sk + Small_bytes, 4, pk, crypto_kem_sntrup761_PUBLICKEYBYTES) < 0) return -1;
  return 0;
}

static int Hide(unsigned char *c, unsigned char *r_enc, const Inputs r, const unsigned char *pk, const unsigned char *cache) {
...
int crypto_kem_sntrup761_enc(unsigned char *c, unsigned char *k, const unsigned char *pk) {
  ...
  if (Short_random(r) < 0) return -1;
  if (Hide(c, r_enc, r, pk, cache) < 0) return -1;
