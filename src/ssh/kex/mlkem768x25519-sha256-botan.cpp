...
{
	try {
		auto *priv = new Botan::ML_KEM_PrivateKey(Botan::system_rng(),
		    Botan::ML_KEM_Mode::ML_KEM_768);
...
static int
mlkem_encaps_botan(uint8_t *ct_out, uint8_t *ss_out,
    const uint8_t *pk, size_t pk_len)
{
	try {
		Botan::ML_KEM_PublicKey pub(
		...
		memcpy(ct_out, result.encapsulated_shared_key().data(),
		    MLKEM_CT_LEN);
		return 0;
	} catch (...) {
		return DSSH_ERROR_INIT;
...
static int
mlkem_decaps_botan(uint8_t *ss_out, const uint8_t *ct,
...
{
	try {
		return hybrid_pq_handler_impl(kctx, &mlkem_params,
		    &botan_mlkem_ops);
	} catch (...) {
		return DSSH_ERROR_INIT;
	}
