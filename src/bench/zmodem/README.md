...
| *(shipped)* | The buffered streaming send producer described in these lessons landed in `sexyz.c` 3.4 (`send_byte` accumulates and hands whole spans to the ring, streaming only; `-w`/`-s` keep the byte-at-a-time path). ~11 → ~115 MB/s, error gate passes. The earlier `fixC` patch that buffered *unconditionally* is gone — it regressed windowed mode, which is why the shipped version gates buffering on streaming. |
