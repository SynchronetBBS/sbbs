...
| `fixC-buffered-producer.patch` | **Not applied — windowed mode is the blocker.** The shipping-shape fix for `sexyz.c` (buffered `send_byte` with transient errors and no dropped bytes; `tx_flush()` before every back-channel wait; a `flush()` that actually reaches the ring instead of being a no-op on the socket path; `SO_SNDBUF` bounded to `OutbufSize`, set before `[sockopts]` so a sysop can override) **plus** a one-line `zmodem_flush()` on `zmodem.c`'s window-full path (`zmodem.c:1720`), which does `continue` back to the back-channel wait without flushing — invisible to a per-byte producer, fatal to a buffered one. Streaming results are excellent: **11.5 → 115.8 MB/s**, sender CPU 44 s → 1.0 s, error gate **3/3** (+32…38 % wire overhead), receive path unchanged (111 MB/s), and *faster* than shipped over a 25 ms link (1.01 vs 0.73 MB/s). **But `-w32768` still regresses: 4.74 MB/s shipped → 1.36 MB/s**, and raising `OutbufSize` to 64 K so the bound exceeds the window makes it *worse* (timeout), so "bound ≥ window" is not the answer. Windowed mode needs its own diagnosis before any of this ships. (`-w8192` fails on the **shipped** sender too — `rc=-8`, nothing transferred — so that one is pre-existing, not a regression.) |
