...
void security_cfg(void)
{
	...
	while (1) {
		...
		uifc.helpbuf =
			...
			"\n"
			"System-wide authentication, password, and account-security policy.\n"
			"Most knobs here have a \"tighten for elevated security at the cost of\n"
			"usability\" axis -- press F1 on individual options for the per-knob\n"
			"guidance.  Changes save immediately; session-level effects apply on\n"
			"next logon.\n"
			"\n"
			"`System Password`: extra password gating sysop functions (entered at\n"
			"the `SY:` prompt and required before privileged commands).  Should be\n"
			"non-guessable and kept confidential -- not the same as user #1's\n"
			"password.\n"
			"\n"
			"`Prompt for System Password`: when (and how often) the system password\n"
			"is required.  `At Login` requires it immediately at sysop login; the\n"
			"timeout sets how many minutes a sysop stays \"vouched\" before being\n"
			"re-prompted.  `0` = always re-prompt.\n"
			"\n"
			"`Allow Sysop Access`: when `No`, accounts at sysop security level lose\n"
			"access to privileged functions entirely (regardless of password).\n"
			"Useful when a sysop wants to use their own account as a regular user.\n"
			"\n"
			"`Allow Login by Real Name`: accept the user's real name (in addition\n"
			"to their alias) at the login prompt.  Disable for elevated security.\n"
			"\n"
			"`Allow Login by User Number`: accept the numeric user number at the\n"
			"login prompt.  Disable for elevated security.\n"
			"\n"
			"`Users Can/Must Change Password`: whether users may pick their own\n"
			"passwords, the minimum length, and whether they're forced to rotate\n"
			"periodically.  Sub-prompts cover the related minimum-length and\n"
			"maximum-password-age knobs.\n"
			"\n"
			"`Demand High Quality Password`: require an entropy-based \"quality\"\n"
			"check on user-chosen passwords.\n"
			"\n"
			"`Always Prompt for Password`: even when the login name is unknown,\n"
			"still ask for a password.  Reduces user-enumeration leakage.\n"
			"\n"
			"`Display/Log Passwords Locally`: write passwords to the local console\n"
			"and logs (e.g. on failed logins).  Set `No` for elevated security.\n"
			"\n"
			"`Days to Preserve Deleted Users`: how long a deleted user slot is\n"
			"kept before being reusable for a new signup.\n"
			"\n"
			"`Maximum Days of User Inactivity`: auto-delete accounts that haven't\n"
			"logged on for this many days.  `0` = unlimited.  Accounts with the\n"
			"`P` (permanent) exemption are never auto-deleted.\n"
			"\n"
			"`Open to New Users`: allow self-registration via the `New` login.  If\n"
			"set to `Yes` with a `New User Password`, only callers who know that\n"
			"password may register; blank = anyone may register.  `No` closes\n"
			"registration entirely.\n"
			"\n"
			"`User Expires When Out-of-time`: when a user runs out of time online,\n"
			"apply the Expired Account Values to their account.  Rarely enabled.\n"
			"\n"
			"`Create Self-signed Certificate`: when no TLS certificate is found,\n"
			"automatically generate a self-signed one for TLS-using servers.\n"
			"\n"
			"`Security Level Values...`: per-level resource limits (time per day,\n"
			"posts per day, downloads, free credits, expiration target).\n"
			"\n"
			"`Expired Account Values...`: the security level, flags removed, and\n"
			"restrictions added when an account expires.\n"
			"\n"
			"`Quick-Validation Values...`: ten preset (level + flags + extras)\n"
			"combinations selectable from UEDIT for fast user validation.\n"
		;
...
void sys_cfg(void)
{
	...
	while (1) {
		...
		uifc.helpbuf =
			...
			"\n"
			"System-wide identity, time/date defaults, and entry points to most\n"
			"other BBS-wide settings.  Changes take effect immediately on save (no\n"
			"server recycle needed for these), though running sessions may still\n"
			"display the old values until the user logs off.\n"
			"\n"
			"`BBS Name`: name displayed at logon, in welcome banners, headers, and\n"
			"most other places that identify \"your\" system.\n"
			"\n"
			"`Location`: free-form text shown in the BBS name banner and user lists\n"
			"(e.g. `Seattle, WA`).  Used for display only.\n"
			"\n"
			"`Local Time Zone`: the system's time zone, used to convert UTC timestamps\n"
			"for display, schedule timed events, and stamp outgoing messages.\n"
			"`Auto` follows the host OS's local zone (with auto-DST when applicable).\n"
			"\n"
			"`Short Date Format`: how dates are rendered to users in compact form\n"
			"(menus, headers).  The \"display\" preview shows the current date in\n"
			"the selected format.\n"
			"\n"
			"`Operator`: the sysop's display name (the person notified about pages,\n"
			"new-user signups, etc.).  Often the same as user #1's alias.\n"
			"\n"
			"`Notifications...`: which sysop activities (new users, file uploads,\n"
			"pages, etc.) generate notifications, and how they're delivered.\n"
			"\n"
			"`Toggle Options...`: a list of yes/no BBS-wide behaviors (aliases,\n"
			"time-banking, credit conversions, military time, etc.).\n"
			"\n"
			"`New User Values...`: defaults applied to fresh user accounts (initial\n"
			"security level, credits, time, etc.).\n"
			"\n"
			"`New User Prompts...`: which fields a new user is asked to fill in\n"
			"during account creation, and whether each is required.\n"
			"\n"
			"`Security Options...`: password requirements, expiration policy, user\n"
			"expiration, and other account-security knobs.\n"
			"\n"
			"`Advanced Options...`: less-commonly-changed settings (logfile paths,\n"
			"inactivity timeouts, sound, MQTT, etc.).\n"
			"\n"
			"`Loadable Modules...`: per-event JavaScript modules invoked at logon,\n"
			"logoff, new-user creation, and other lifecycle points.\n"
			"\n"
			"`Extra Attribute Codes...`: enable support for other BBS programs'\n"
			"color-code syntaxes (WWIV, PCBoard, Wildcat, Celerity, Renegade) in\n"
			"display files, so menus authored for those systems render correctly.\n"
		;
			...
			case 7:    /* New User Values */
				...
				while (!done) {
					...
					uifc.helpbuf =
						"`New User Values:`\n"
						"\n"
						"Defaults applied to every fresh user account at registration.  Each\n"
						"value can be edited later via UEDIT for individual users; this menu\n"
						"sets the starting state for everyone.  Also applied automatically to\n"
						"`Guest` (G-restricted) account logons.\n"
						"\n"
						"`Level`: starting security level (0-99).  Recommended `50` or less;\n"
						"levels 90-99 are reserved for sysops and co-sysops.\n"
						"\n"
						"`Flag Set #1` to `#4`: starting security flags from each of the four\n"
						"flag sets, granting access to flag-gated sub-boards, dirs, externals,\n"
						"etc.  Blank = no flags granted.\n"
						"\n"
						"`Exemptions`: starting exemptions (single letters like `T`, `M`, `D`,\n"
						"...) that override per-level limits (time, mail, downloads, ...).\n"
						"Blank = no exemptions.\n"
						"\n"
						"`Restrictions`: starting restrictions (single letters) that block\n"
						"specific abilities (`C` = no chat, `M` = no mail, etc.).\n"
						"Blank = no restrictions.\n"
						"\n"
						"`Expiration Days`: number of days until a new account expires.  `0`\n"
						"disables expiration (account is permanent unless deleted).\n"
						"\n"
						"`Credits`: starting credit balance.  Default `10,485,760` (10 MiB /\n"
						"$5 at default rate) gives a new user a comfortable starting download\n"
						"budget.\n"
						"\n"
						"`Minutes`: starting minutes in the user's minute bank.  Per-session\n"
						"time comes from the security level's `Time Per Call`, not from this.\n"
						"\n"
						"`Editor`: default external full-screen message editor (must be one of\n"
						"the editors configured under `External Programs > External Editors`).\n"
						"Blank = use the built-in line editor.\n"
						"\n"
						"`Command Shell`: default command shell for new users -- Baja or\n"
						...
						"in a new user's first new-message scan.  `0` = no backfill.\n"
						"\n"
						"`Gender Options`: single-character codes the new-user gender prompt\n"
						"accepts (e.g. `MFX`).  Add or remove letters to change the choices.\n"
						"\n"
						"`Default Toggles...`: per-feature on/off defaults applied to new\n"
						"users (screen pause, mouse, message scan options, etc.).\n"
						"\n"
						"`QWK Packet Settings...`: defaults for the QWK offline-reader packet\n"
						"format (Ctrl-A handling, attachments, kludges, etc.).\n"
					;
			...
			case 10:    /* Advanced Options */
				...
				while (!done) {
					...
					uifc.helpbuf =
						...
						"\n"
						"Less-commonly-changed knobs for the BBS as a whole: directory paths,\n"
						"log-file rotation, inactivity timeouts, statistic-cache intervals, etc.\n"
						"Most defaults are sensible; press F1 on an individual option for full\n"
						"detail.  Changes save immediately but some take effect only after a\n"
						"Terminal Server recycle (paths and timeouts in particular).\n"
						"\n"
						"`New User Magic Word`: secret word a new user must echo back after\n"
						"registration; placed in `text/newuser.msg` to verify they read it.\n"
						"\n"
						"`Data Directory`, `Logs Directory`, `Exec Directory`, `Mods Directory`:\n"
						"filesystem paths for the four canonical Synchronet directories.  Data\n"
						"must be on shared, non-volatile storage accessible to all nodes.  Mods\n"
						"overlays Exec for sysop customizations that survive upgrades.\n"
						"\n"
						"`Input SIF Questionnaire` / `Output SIF Questionnaire`: optional SIF\n"
						"files (in `text/`) presenting questions to new users and to the sysop\n"
						"during user-edit review.\n"
						"\n"
						"`Credits Per Dollar`: how many bytes of credit one dollar represents.\n"
					"Credits are a byte-denominated download budget.  Accepts byte\n"
					"suffixes (`K`/`M`/`G`); default `2M` (2 MiB / dollar) is conventional.\n"
						"\n"
						"`Minutes Per 100K Credits`: how many minutes of time a user gets when\n"
						"converting a 100 KiB block of credits to time.\n"
						"\n"
						"`Maximum Number of Minutes`: cap on the total minutes any user may\n"
						"hold in their minute bank.  `0` = unlimited.\n"
						"\n"
						"`Warning Days Till Expire`: how many days before account expiration to\n"
						"start warning the user at logon.  `0` disables the warning.\n"
						"\n"
						"`Last Displayable Node`: highest node number shown to users in node\n"
						"lists; higher nodes are hidden.\n"
						"\n"
						"`Phone Number Format`: template for new-user phone-number entry\n"
						"(`N` = digit, `A` = alpha, `!` = any; other chars are literal).\n"
						"\n"
						"`Sysop Chat Override`: ARS describing which users may page the sysop\n"
						"regardless of sysop availability.\n"
						"\n"
						"`User Database Backups`, `Mail Database Backups`, `Configuration Backups`:\n"
						"number of daily backups to retain.  `0` = disabled; `5` is typical.\n"
						"\n"
						"`Maximum Log File Size`: size at which a log rotates to a numbered\n"
						"archive (and oldest archives are dropped to stay under the kept count).\n"
						"`Unlimited` disables rotation.\n"
						"\n"
						"`Maximum User Inactivity`: keyboard idle time at a prompt before a\n"
						"user is auto-disconnected (independent of the lower-level socket\n"
						"timeouts under `Terminal Server`).\n"
						"\n"
						"`User Inactivity Warning`: percent of the inactivity limit elapsed\n"
						"before the user is warned of imminent disconnect.  `0` suppresses all\n"
						"warnings.\n"
						"\n"
						"`Control Key Pass-through`: 32-bit mask of Ctrl-letter keys that\n"
						"Synchronet should ignore (pass through to running programs).  Bit n\n"
						"corresponds to Ctrl-key with ASCII value n; e.g. `8` ignores Ctrl-C.\n"
						"\n"
						"`Statistics Interval`: how often statistics files are re-read for\n"
						"display to users.  Lower = fresher numbers, more disk I/O.\n"
						"\n"
						"`Cache Filter Files`: how often `text/*.can` filter files are checked\n"
						"for changes; in-memory cache between checks reduces I/O on hot paths.\n"
						"`0` disables caching.\n"
					;
					switch (uifc.list(WIN_ACT | WIN_BOT | WIN_RHT | WIN_SAV, 0, 0, 0, &adv_dflt, &adv_bar
						...
						case 24:
							uifc.helpbuf =
								...
								"to a user's terminal.  A larger file plays only if `@CACHE_AUDIO:`\n"
								"has already sent it; otherwise it is skipped and the reason logged\n"
								"once per file.\n"
								"\n"
