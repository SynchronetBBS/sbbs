...
*/
		...
		truncsp(str);
#if 0
		if((cfg.node_path=(char **)REALLOC(cfg.node_path
			,sizeof(char *)*(cfg.sys_nodes+1)))
			==NULL) {
			errormsg(WHERE,ERR_ALLOC,nulstr,cfg.sys_nodes+1);
			continue; }
		if((cfg.node_path[i-1]=MALLOC(strlen(str)+2))==NULL) {
...
while(1) {
	i=0;
	sprintf(opt[i++],"%-27.27s%s","Name",cfg.node_name);
	sprintf(opt[i++],"%-27.27s%s","Phone Number",cfg.node_phone);
	sprintf(opt[i++],"%-27.27s%ubps","Minimum Connect Rate",cfg.node_minbps);
	sprintf(opt[i++],"%-27.27s%.40s","Logon Requirements",cfg.node_arstr);
	sprintf(opt[i++],"%-27.27s%.40s","Local Text Editor",cfg.node_editor);
	sprintf(opt[i++],"%-27.27s%.40s","Local Text Viewer",cfg.node_viewer);
	sprintf(opt[i++],"%-27.27s%.40s","Configuration Command",cfg.scfg_cmd);
	sprintf(opt[i++],"%-27.27s%.40s","DOS Command Interpreter"
		,cfg.node_comspec[0] ? cfg.node_comspec : "%COMSPEC%");
...
purposes only.
*/
			uinput(WIN_MID|WIN_SAV,0,10,"Name",cfg.node_name,40,K_EDIT);
			break;
		case 1:
SETHELP(WHERE);
/*
...
*/
			uinput(WIN_MID|WIN_SAV,0,10,"Phone Number",cfg.node_phone,12,K_EDIT);
			break;
		case 2:
SETHELP(WHERE);
/*
...
*/
			uinput(WIN_MID|WIN_SAV,0,13,"Editor",cfg.node_editor
				,50,K_EDIT);
			break;
		case 5:
SETHELP(WHERE);
...
*/
			uinput(WIN_MID|WIN_SAV,0,14,"Viewer",cfg.node_viewer
				,50,K_EDIT);
			break;
		case 6:
SETHELP(WHERE);
...
*/
			uinput(WIN_MID|WIN_SAV,0,14,"Configuration Command",cfg.scfg_cmd
				,40,K_EDIT);
            break;
		case 7:
SETHELP(WHERE);
		...
		case 8:
			...
			while(!done) {
				...
				sprintf(opt[i++],"%-27.27s%s","Disable Local Inactivity"
					,cfg.node_misc&NM_NO_INACT ? "Yes":"No");
				sprintf(opt[i++],"%-27.27s%s","Disable Local Keyboard"
                    ,cfg.node_misc&NM_NO_LKBRD ? "Yes":"No");
				sprintf(opt[i++],"%-27.27s%s","Local System Protection"
					,cfg.node_misc&NM_SYSPW ? "Yes":"No");
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_ANSALARM)) {
							cfg.node_misc|=NM_ANSALARM;
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_ANSALARM)) {
							cfg.node_misc&=~NM_ANSALARM;
							changes=1; }
						break;
					case 1:
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_WFCSCRN)) {
							cfg.node_misc|=NM_WFCSCRN;
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_WFCSCRN)) {
							cfg.node_misc&=~NM_WFCSCRN;
							changes=1; }
						break;
					case 2:
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_WFCMSGS)) {
							cfg.node_misc|=(NM_WFCMSGS|NM_WFCSCRN);
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_WFCMSGS)) {
							cfg.node_misc&=~NM_WFCMSGS;
							changes=1; }
                        break;
					case 3:
						i=1;
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_LCL_EDIT)) {
							cfg.node_misc|=NM_LCL_EDIT;
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_LCL_EDIT)) {
							cfg.node_misc&=~NM_LCL_EDIT;
							changes=1; }
						break;
					case 4:
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_EMSOVL)) {
							cfg.node_misc|=NM_EMSOVL;
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_EMSOVL)) {
							cfg.node_misc&=~NM_EMSOVL;
							changes=1; }
						break;
					case 5:
...
*/
						i=ulist(WIN_MID|WIN_SAV,0,10,0,&i,0
							,"Allow Swapping",opt);
						if(i==0 && cfg.node_swap&SWAP_NONE) {
							cfg.node_swap&=~SWAP_NONE;
							changes=1; }
						else if(i==1 && !(cfg.node_swap&SWAP_NONE)) {
							cfg.node_swap|=SWAP_NONE;
							changes=1; }
                        break;
					case 6:
...
*/
						...
						if(i==1 && cfg.node_swap&SWAP_EMS) {
							cfg.node_swap&=~SWAP_EMS;
							changes=1; }
						else if(i==0 && !(cfg.node_swap&SWAP_EMS)) {
							cfg.node_swap|=SWAP_EMS;
							changes=1; }
                        break;
					case 7:
...
*/
						...
						if(i==1 && cfg.node_swap&SWAP_XMS) {
							cfg.node_swap&=~SWAP_XMS;
							changes=1; }
						else if(i==0 && !(cfg.node_swap&SWAP_XMS)) {
							cfg.node_swap|=SWAP_XMS;
							changes=1; }
                        break;
					case 8:
...
*/
						...
						if(i==1 && cfg.node_swap&SWAP_EXT) {
							cfg.node_swap&=~SWAP_EXT;
							changes=1; }
						else if(i==0 && !(cfg.node_swap&SWAP_EXT)) {
							cfg.node_swap|=SWAP_EXT;
							changes=1; }
                        break;
					case 9:
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_WINOS2)) {
							cfg.node_misc|=NM_WINOS2;
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_WINOS2)) {
							cfg.node_misc&=~NM_WINOS2;
							changes=1; }
                        break;
					case 10:
...
*/
						...
						if(i==0 && cfg.node_misc&NM_NODV) {
							cfg.node_misc&=~NM_NODV;
							changes=1; }
						else if(i==1 && !(cfg.node_misc&NM_NODV)) {
							cfg.node_misc|=NM_NODV;
							changes=1; }
                        break;
					case 11:
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_INT28)) {
							cfg.node_misc|=NM_INT28;
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_INT28)) {
							cfg.node_misc&=~NM_INT28;
							changes=1; }
                        break;
					case 12:
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_NO_INACT)) {
							cfg.node_misc|=NM_NO_INACT;
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_NO_INACT)) {
							cfg.node_misc&=~NM_NO_INACT;
							changes=1; }
                        break;
					case 17:
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_NO_LKBRD)) {
							cfg.node_misc|=NM_NO_LKBRD;
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_NO_LKBRD)) {
							cfg.node_misc&=~NM_NO_LKBRD;
							changes=1; }
                        break;
					case 18:
...
*/
						...
						if(i==0 && !(cfg.node_misc&NM_SYSPW)) {
							cfg.node_misc|=NM_SYSPW;
							changes=1; }
						else if(i==1 && (cfg.node_misc&NM_SYSPW)) {
							cfg.node_misc&=~NM_SYSPW;
							changes=1; }
                        break;
					case 19:
...
*/
						i=ulist(WIN_MID|WIN_SAV,0,10,0,&i,0
							,"Beep Locally",opt);
						if(i==1 && !(cfg.node_misc&NM_NOBEEP)) {
							cfg.node_misc|=NM_NOBEEP;
							changes=1; }
						else if(i==0 && (cfg.node_misc&NM_NOBEEP)) {
							cfg.node_misc&=~NM_NOBEEP;
							changes=1; }
                        break;
					case 20:
		...
		case 9:
			...
			while(!done) {
				...
				sprintf(opt[i++],"%-27.27s%s","Screen Length"
					,cfg.node_scrnlen ? itoa(cfg.node_scrnlen,tmp,10)
					: "Auto-Detect");
				sprintf(opt[i++],"%-27.27s%s","Screen Blanker"
					,cfg.node_scrnblank ? itoa(cfg.node_scrnblank,tmp,10)
					: "Disabled");
...
*/
						...
						cfg.node_scrnlen=atoi(str);
						break;
					case 2:
						itoa(cfg.node_scrnblank,str,10);
SETHELP(WHERE);
/*
Screen Blanker:
...
*/
                        ...
                        break;
					case 7:
						itoa(cfg.node_dollars_per_call,str,10);
SETHELP(WHERE);
/*
...
*/
						...
						cfg.node_dollars_per_call=atoi(str);
						break;
					case 8:
SETHELP(WHERE);
/*
...
*/
						...
						if(!strlen(str))
							...
							strcpy(cfg.temp_dir,str);
						break;
					case 12:
						strcpy(str,cfg.node_swapdir);
SETHELP(WHERE);
/*
Swap Directory:
...
*/
			...
			while(1) {
				for(i=0;i<10;i++)
					sprintf(opt[i],"%d  %s",i,cfg.wfc_cmd[i]);
				opt[i][0]=0;
				savnum=0;
				...
				uinput(WIN_MID|WIN_SAV,0,0,str,cfg.wfc_cmd[i],50,K_EDIT); }
			break;
		case 12:
SETHELP(WHERE);
/*
...
*/
			...
			while(1) {
				for(i=0;i<12;i++)
					sprintf(opt[i],"F%-2d  %s",i+1,cfg.wfc_scmd[i]);
				opt[i][0]=0;
				savnum=0;
				i=ulist(WIN_T2B|WIN_RHT|WIN_ACT|WIN_SAV,2,0,50,&j,0
				...
				uinput(WIN_MID|WIN_SAV,0,0,str,cfg.wfc_scmd[i],50,K_EDIT); }
            break;
		case 10:
			done=0;
			while(!done) {
				...
				sprintf(opt[i++],"%-27.27s%s","COM Port"
					,cfg.com_port ? itoa(cfg.com_port,tmp,10) : "Disabled");
				sprintf(opt[i++],"%-27.27s%u"
					,cfg.com_base && cfg.com_base<0x100 ? "COM Channel"
						:"UART IRQ Line"
					,cfg.com_irq);
				if(cfg.com_base==0xd)
					strcpy(str,"DigiBoard");
				else if(cfg.com_base==0xf)
					strcpy(str,"FOSSIL");
				else if(cfg.com_base==0xb)
					strcpy(str,"PC BIOS");
				else if(cfg.com_base==0xe)
					strcpy(str,"PS/2 BIOS");
				else
					sprintf(str,"%Xh",cfg.com_base);
				sprintf(opt[i++],"%-27.27s%s"
					,cfg.com_base<0x100 ? "COM Type":"UART I/O Address",str);
				sprintf(opt[i++],"%-27.27s%lu","DTE Rate",cfg.com_rate);
				sprintf(opt[i++],"%-27.27s%s","Fixed DTE Rate"
					,cfg.mdm_misc&MDM_STAYHIGH ? "Yes" : "No");
				if(!(cfg.mdm_misc&(MDM_RTS|MDM_CTS)))
					strcpy(str,"None");
				else {
					if(cfg.mdm_misc&MDM_CTS) {
						strcpy(str,"CTS");
						if(cfg.mdm_misc&MDM_RTS)
							strcat(str,"/"); }
					else str[0]=0;
					if(cfg.mdm_misc&MDM_RTS)
						strcat(str,"RTS"); }
				...
				sprintf(opt[i++],"%-27.27s%u ring%c","Answer After"
					,cfg.mdm_rings,cfg.mdm_rings>1 ? 's':' ');
				sprintf(opt[i++],"%-27.27s%u second%c","Answer Delay"
					,cfg.mdm_ansdelay,cfg.mdm_ansdelay>1 ? 's':' ');
				sprintf(opt[i++],"%-27.27s%s","Reinitialization Timer"
					,cfg.mdm_reinit ? itoa(cfg.mdm_reinit,tmp,10) : "Disabled");
...
*/
						uinput(WIN_MID|WIN_SAV,0,10
							,cfg.com_base && cfg.com_base<0x100 ? "Channel"
							: "UART IRQ Line"
							,str,2,K_EDIT|K_NUMBER);
						cfg.com_irq=atoi(str);
						break;
					case 2:
						savnum=1;
						if(com_type() || cfg.com_base<0x100)
							break;
SETHELP(WHERE);
/*
...
*/
						itoa(cfg.com_base,str,16);
						strupr(str);
						uinput(WIN_MID|WIN_SAV,0,10
							,"UART I/O Address in Hex"
							,str,4,K_EDIT|K_UPPER);
						cfg.com_base=ahtoul(str);
                        break;
					case 3:
						savnum=1;
SETHELP(WHERE);
...
*/
						i=ulist(WIN_MID|WIN_SAV,0,0,0,&i,0,"Use Fixed "
							"DTE Rate",opt);
						if(i==0 && !(cfg.mdm_misc&MDM_STAYHIGH)) {
							cfg.mdm_misc|=MDM_STAYHIGH;
							changes=1; }
						else if(i==1 && cfg.mdm_misc&MDM_STAYHIGH) {
							cfg.mdm_misc&=~MDM_STAYHIGH;
							changes=1; }
						break;
					case 5:
...
*/
						...
						if(i==0
							&& (cfg.mdm_misc&(MDM_CTS|MDM_RTS))
									   !=(MDM_CTS|MDM_RTS)) {
							cfg.mdm_misc|=(MDM_CTS|MDM_RTS);
							changes=1; }
						else if(i==1
							&& (cfg.mdm_misc&(MDM_CTS|MDM_RTS))!=MDM_CTS) {
							cfg.mdm_misc|=MDM_CTS;
							cfg.mdm_misc&=~MDM_RTS;
							changes=1; }
						else if(i==2
							&& (cfg.mdm_misc&(MDM_CTS|MDM_RTS))!=MDM_RTS) {
							cfg.mdm_misc|=MDM_RTS;
							cfg.mdm_misc&=~MDM_CTS;
                            changes=1; }
						else if(i==3
...
*/
                        itoa(cfg.mdm_rings,str,10);
                        uinput(WIN_MID|WIN_SAV,0,0
                            ,"Number of Rings to Answer After"
                            ,str,2,K_EDIT|K_NUMBER);
                        cfg.mdm_rings=atoi(str);
                        if(!cfg.mdm_rings)
                            cfg.mdm_rings=1;
                        break;
...
*/
						itoa(cfg.mdm_ansdelay,str,10);
						uinput(WIN_MID|WIN_SAV,0,0
							,"Answer Delay (in seconds)"
							,str,4,K_EDIT|K_NUMBER);
						cfg.mdm_ansdelay=atoi(str);
						break;
...
*/
						itoa(cfg.mdm_reinit,str,10);
						uinput(WIN_MID|WIN_SAV,0,0
							,"Minutes Between Reinitialization (0=Disabled)"
							,str,4,K_EDIT|K_NUMBER);
						cfg.mdm_reinit=atoi(str);
						break;
					case 9:
						dflt=bar=0;
						while(1) {
							for(i=0;i<cfg.mdm_results;i++) {
								sprintf(opt[i],"%3u %5ubps %4ucps %s"
									,cfg.mdm_result[i].code,cfg.mdm_result[i].rate
									,cfg.mdm_result[i].cps,cfg.mdm_result[i].str); }
							opt[i][0]=0;
							i=WIN_ACT|WIN_SAV|WIN_RHT;
							if(cfg.mdm_results<MAX_OPTS)
								i|=WIN_INS;
							if(cfg.mdm_results>1)
								i|=WIN_DEL;
							savnum=1;
SETHELP(WHERE);
/*
...
*/
							...
							if((i&MSK_ON)==MSK_DEL) {
								i&=MSK_OFF;
								cfg.mdm_results--;
								while(i<cfg.mdm_results) {
									cfg.mdm_result[i]=cfg.mdm_result[i+1];
									i++; }
								changes=1;
								continue; }
							if((i&MSK_ON)==MSK_INS) {
								...
								if((cfg.mdm_result=(mdm_result_t *)REALLOC(
									...
									*(cfg.mdm_results+1)))==NULL) {
									...
									bail(1);
									continue; }
								for(j=cfg.mdm_results;j>i;j--)
									cfg.mdm_result[j]=cfg.mdm_result[j-1];
								cfg.mdm_result[i]=cfg.mdm_result[i+1];
								cfg.mdm_results++;
								changes=1;
								continue; }
					...
					case 10:	/* Toggle Options */
						...
						while(1) {
							...
							sprintf(opt[i++],"%-27.27s%s"
								...
								,cfg.mdm_misc&MDM_DUMB ? "Yes" : "No");
							sprintf(opt[i++],"%-27.27s%s"
								,"Drop DTR to Hang Up"
								,cfg.mdm_misc&MDM_NODTR ? "No" : "Yes");
							sprintf(opt[i++],"%-27.27s%s"
								,"Use Verbal Result Codes"
								,cfg.mdm_misc&MDM_VERBAL ? "Yes" : "No");
							sprintf(opt[i++],"%-27.27s%s"
								,"Allow Unknown Result Codes"
                                ,cfg.mdm_misc&MDM_KNOWNRES ? "No" : "Yes");
							opt[i][0]=0;
...
*/
								i=ulist(WIN_MID|WIN_SAV,0,0,0,&i,0
									,"Caller ID",opt);
								if(i==0 && !(cfg.mdm_misc&MDM_CALLERID)) {
									cfg.mdm_misc|=MDM_CALLERID;
									changes=1; }
								else if(i==1 && cfg.mdm_misc&MDM_CALLERID) {
									cfg.mdm_misc&=~MDM_CALLERID;
									changes=1; } }
							else if(i==1) {
								i=1;
...
*/
								i=ulist(WIN_SAV|WIN_MID,0,0,0,&i,0
									,"Dumb Modem",opt);
								if(i==0 && !(cfg.mdm_misc&MDM_DUMB)) {
									cfg.mdm_misc|=MDM_DUMB;
									changes=1; }
								else if(i==1 && cfg.mdm_misc&MDM_DUMB) {
									cfg.mdm_misc&=~MDM_DUMB;
									changes=1; } }
							else if(i==2) {
								i=0;
...
*/
								...
								if(i==0 && cfg.mdm_misc&MDM_NODTR) {
									cfg.mdm_misc&=~MDM_NODTR;
									changes=1; }
								else if(i==1 && !(cfg.mdm_misc&MDM_NODTR)) {
									cfg.mdm_misc|=MDM_NODTR;
                                    changes=1; } }
							else if(i==3) {
								i=1;
...
*/
								...
								if(i==0 && !(cfg.mdm_misc&MDM_VERBAL)) {
									cfg.mdm_misc|=MDM_VERBAL;
									changes=1; }
								else if(i==1 && cfg.mdm_misc&MDM_VERBAL) {
									cfg.mdm_misc&=~MDM_VERBAL;
                                    changes=1; } }
							else if(i==4) {
								i=0;
...
*/
								...
								if(i==0 && cfg.mdm_misc&MDM_KNOWNRES) {
									cfg.mdm_misc&=~MDM_KNOWNRES;
									changes=1; }
								else if(i==1 && !(cfg.mdm_misc&MDM_KNOWNRES)) {
									cfg.mdm_misc|=MDM_KNOWNRES;
                                    changes=1; } }
									}
						break;
...
*/
                        ...
                        while(1) {
							...
							sprintf(opt[i++],"%-27.27s%.40s"
                                ,"Initialization String",cfg.mdm_init);
							sprintf(opt[i++],"%-27.27s%.40s"
                                ,"Special Init String",cfg.mdm_spec);
							sprintf(opt[i++],"%-27.27s%.40s"
                                ,"Terminal Init String",cfg.mdm_term);
							sprintf(opt[i++],"%-27.27s%.40s"
                                ,"Dial String",cfg.mdm_dial);
							sprintf(opt[i++],"%-27.27s%.40s"
                                ,"Off Hook String",cfg.mdm_offh);
							sprintf(opt[i++],"%-27.27s%.40s"
                                ,"Answer String",cfg.mdm_answ);
							sprintf(opt[i++],"%-27.27s%.40s"
								,"Hang Up String",cfg.mdm_hang);
							opt[i][0]=0;
...
*/
                                    uinput(WIN_MID|WIN_SAV,0,0
                                        ,"Initialization"
                                        ,cfg.mdm_init,50,K_EDIT|K_UPPER);
                                    break;
                                case 1:
SETHELP(WHERE);
/*
...
*/
                                    uinput(WIN_MID|WIN_SAV,0,0
                                        ,"Special Init"
                                        ,cfg.mdm_spec,50,K_EDIT|K_UPPER);
                                    break;
                                case 2:
SETHELP(WHERE);
/*
...
*/
                                    uinput(WIN_MID|WIN_SAV,0,0
                                        ,"Terminal Init"
                                        ,cfg.mdm_term,50,K_EDIT|K_UPPER);
                                    break;
                                case 3:
SETHELP(WHERE);
/*
...
*/
                                    uinput(WIN_MID|WIN_SAV,0,0
										,"Hang Up String",cfg.mdm_hang,40
                                        ,K_EDIT|K_UPPER);
                                    break;
									} }
						break;
...
while(1) {
	n=0;
	sprintf(opt[n++],"%-22.22s%u","Numeric Code",cfg.mdm_result[i].code);
	sprintf(opt[n++],"%-22.22s%ubps","Modem Connect Rate",cfg.mdm_result[i].rate);
	sprintf(opt[n++],"%-22.22s%ucps","Modem Average CPS",cfg.mdm_result[i].cps);
	sprintf(opt[n++],"%-22.22s%s","Description",cfg.mdm_result[i].str);
	opt[n][0]=0;
	savnum=2;
SETHELP(WHERE);
/*
...
*/
	switch(ulist(WIN_SAV|WIN_MID|WIN_ACT,0,0,35,&dflt,0,"Result Code",opt)) {
		case -1:
			return;
		case 0:
			sprintf(str,"%u",cfg.mdm_result[i].code);
SETHELP(WHERE);
/*
Numeric Result Code:
...
*/
			uinput(WIN_L2R|WIN_SAV,0,15,"Numeric Result Code",str,3
				,K_EDIT|K_NUMBER);
			cfg.mdm_result[i].code=atoi(str);
			break;
		case 1:
SETHELP(WHERE);
/*
...
*/
			sprintf(str,"%u",cfg.mdm_result[i].rate);
			uinput(WIN_L2R|WIN_SAV,0,15,"Connection (DCE) Rate",str,5
				,K_EDIT|K_NUMBER);
			cfg.mdm_result[i].rate=atoi(str);
			break;
		case 2:
SETHELP(WHERE);
/*
...
*/
			sprintf(str,"%u",cfg.mdm_result[i].cps);
			uinput(WIN_L2R|WIN_SAV,0,15,"Average File Transfer CPS",str,5
				,K_EDIT|K_NUMBER);
			cfg.mdm_result[i].cps=atoi(str);
			break;
		case 3:
SETHELP(WHERE);
/*
...
*/
			uinput(WIN_L2R|WIN_SAV,0,15,"Modem Description"
				,cfg.mdm_result[i].str,LEN_MODEM,K_EDIT);
			break; } }
}
