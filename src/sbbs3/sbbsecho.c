...
void pack_bundle(char *infile,faddr_t dest)
{
	...
	if(node<cfg.nodecfgs)
		if(cfg.nodecfg[node].arctype==SBBSECHO_ARCTYPE_NONE) {    /* Uncompressed! */
			if(misc&FLO_MAILER)
...
void bail(int code)
{
	...
	for(u=0; locked_bso_nodes[u]!=NULL; u++)
		delfile(locked_bso_nodes[u], __LINE__);
