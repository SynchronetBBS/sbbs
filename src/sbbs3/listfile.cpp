...
{
	char	str[256],hdr[256],c,d,letter='A',*p,*datbuf;
	char 	tmp[512];
	uchar	flagprompt=0;
	uint	i,j;
	...
	while(online) {
		...
		if(m>=file_count || flagprompt) {		  /* End of list */
			if(useron.misc&BATCHFLAG && !tofile && found && found!=lastbat
				...
				flagprompt=0;
				lncntr=0;
				if((i=batchflagprompt(&smb, bf, letter-'A', file_count))==2) {
					m=anchor;
					found-=letter-'A';
					letter='A'; 
				}
...
**/
		...
		if(useron.misc&BATCHFLAG && !tofile) {
			...
			if(flagprompt || letter=='Z' || !disp ||
				...
				lncntr=0;
				lastbat=found;
				if((int)(i=batchflagprompt(&smb, bf, letter-'A'+1, file_count))<1) {
					freefiles(file_list, file_count);
					smb_close(&smb);
					if((int)i==-1)
						return(-1);
...
/* or 3, backwards. 														*/
/****************************************************************************/
int sbbs_t::batchflagprompt(smb_t* smb, smbfile_t* bf, uint total
							,long totalfiles)
{
...
//							padfname(str+c,tmp);
							if(filematch(bf[i].filename,tmp)) {
#if 0
...
{
	char	str[258],path[258],dirpath[256],done=0,ch,ext[513];
	char 	tmp[512];
	uchar	*p;
	int		file;
	int		error;
			...
			switch(getkeys(str,0)) {
				...
				case 'E':   /* edit file information */
					if(dir_op(dirnum)) {
						...
						SAFECOPY(str, f->filename);
						if(!getstr(str,12,K_EDIT|K_AUTODEL))
							break;
						if(strcmp(str,f->filename) != 0) { /* rename */
//							padfname(str,path);
...
#endif
									::removefile(&smb, f);
									smb_addfile(&smb, f, SMB_SELFPACK, f->extdesc);
								} 
							} 
						} 
					}
					...
					getstr(fdesc, sizeof(fdesc) ,K_LINE|K_EDIT|K_AUTODEL);
					if(sys_status&SS_ABORT)
						break;
					if(f->extdesc != NULL && *f->extdesc) {
						if(!noyes(text[DeleteExtDescriptionQ])) {
/*							remove(str); <- wow, this is an old bug */
//							f.misc&=~FM_EXTDESC; 
						} 
					}
					if(!dir_op(dirnum)) {
...
//					f.cdt=atol(str);
					...
					if(sys_status&SS_ABORT)
						break;
//					putfiledat(&cfg,&f);
//					inputnstime32(&f->hdr.last_downloaded);
//					update_uldate(&cfg, &f);
					renewfile(&smb, f);
					break;
	...
	/* Fall through */      break; 
				...
				case 'C':   /* remove credits only */
					...
					sprintf(str,text[FileRemovedUserMsg]
						,f->filename,f->cost ? ultoac(f->cost,tmp) : text[No]);
					putsmsg(&cfg,i,str);
...
//					closefile(&f);
				...
				if(i<cfg.total_prots) {
					...
					const char* file_ext = getfext(f->filename);
					if(file_ext != NULL) {
