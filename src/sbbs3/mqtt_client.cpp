...
extern "C" void *mqtt5client_open(void)
{
	return new mqtt5::Client();
}

extern "C" void mqtt5client_close(void *handle)
{
	delete static_cast<mqtt5::Client *>(handle);
}

extern "C" void mqtt5client_set_will(void *handle, const char *topic,
                                         const void *payload, size_t len,
                                         int qos, int retain)
{
	if (handle)
		static_cast<mqtt5::Client *>(handle)->set_will(topic, payload, len, qos, retain != 0);
}

extern "C" int mqtt5client_connect(void *handle, const char *host, uint16_t port,
                                       ...
                                       int (*lprintf)(int level, const char *fmt, ...))
{
	if (!handle)
		return -1;
	return static_cast<mqtt5::Client *>(handle)->connect(
		host, port, client_id, username, password,
		keepalive, 5, tls_mode, psk, psk_identity,
		cafile, certfile, keyfile, keypass, scfg, lprintf);
}

extern "C" void mqtt5client_disconnect(void *handle)
{
	if (handle)
		static_cast<mqtt5::Client *>(handle)->disconnect();
}

extern "C" int mqtt5client_publish(void *handle, const char *topic,
                                       const void *payload, size_t len,
                                       int qos, int retain, const void *props)
{
	if (!handle)
		return -1;
	return static_cast<mqtt5::Client *>(handle)->publish(topic, payload, len, qos, retain != 0,
	                                                      static_cast<const mqtt5::Properties *>(props));
}

extern "C" int mqtt5client_subscribe(void *handle, const char *topic, int qos)
{
	if (!handle)
		return -1;
	return static_cast<mqtt5::Client *>(handle)->subscribe(topic, qos);
}

extern "C" int mqtt5client_pump(void *handle, int timeout_ms)
{
	if (!handle)
		return -1;
	return static_cast<mqtt5::Client *>(handle)->pump(timeout_ms);
}

extern "C" struct mqtt5client_msg *mqtt5client_read(void *handle)
{
	if (!handle)
		return NULL;
	auto *client = static_cast<mqtt5::Client *>(handle);
	mqtt5::ReceivedMessage *rm = client->read(0);
	if (!rm)
		return NULL;
	struct mqtt5client_msg *msg = (struct mqtt5client_msg *)malloc(
		sizeof(struct mqtt5client_msg) + rm->topic.size() + 1 + rm->payload.size());
	if (!msg)
		return NULL;
	char *buf = (char *)(msg + 1);
	memcpy(buf, rm->topic.c_str(), rm->topic.size() + 1);
	msg->topic = buf;
	buf += rm->topic.size() + 1;
	memcpy(buf, rm->payload.data(), rm->payload.size());
	msg->payload = buf;
	msg->payload_len = rm->payload.size();
	return msg;
}

extern "C" void mqtt5client_read_free(struct mqtt5client_msg *msg)
{
	free(msg);
}
