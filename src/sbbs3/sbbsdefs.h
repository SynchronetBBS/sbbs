...
// which these are copied from, but never exceeds MAX_PATH (260 on Windows):
// the rest of the tree builds paths in MAX_PATH+1 buffers, where anything
// longer would silently truncate.  Not simply MAX_PATH either, which is
// PATH_MAX (4096) on Linux: scfg_t is instantiated MAX_NODES times by the
// terminal server (main.cpp's node_scfg[]).
#define LEN_SYSDIR  (MAX_PATH < INI_MAX_VALUE_LEN - 1 ? MAX_PATH : INI_MAX_VALUE_LEN - 1)
