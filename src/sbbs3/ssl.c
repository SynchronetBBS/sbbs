...
{
	/* CRYPTLIB_PATCHES is generated by 3rdp/build/hashpatch.pl as a 32-character
	 * MD5 followed by "  -" (35 chars + NUL = 36 bytes). Catch a build that
	 * leaves it empty before we memcmp 32 bytes off the end of an empty literal. */
	_Static_assert(sizeof(CRYPTLIB_PATCHES) > 32, "CRYPTLIB_PATCHES must be at least 32 chars");
