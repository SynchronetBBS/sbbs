...
bool sbbs_t::postmsg(int subnum, int wm_mode, smb_t* resmb, smbmsg_t* remsg)
{
	...
	if (remsg) {
		SAFECOPY_UTF8(title, msghdr_field(remsg, remsg->subj, NULL, term->supports(UTF8)));
		SAFECOPY(org_title, title);
		if (remsg->hdr.attr & MSG_ANONYMOUS)
			SAFECOPY(from, text[Anonymous]);
		else
			SAFECOPY_UTF8(from, msghdr_field(remsg, remsg->from, NULL, term->supports(UTF8)));
		...
		if (remsg->to != NULL
			...
			SAFECOPY_UTF8(touser, msghdr_field(remsg, remsg->to, NULL, term->supports(UTF8)));
		else
		...
		SAFECOPY(top, format_text(RegardingByToOn, remsg
		                          , title
		                          , from
		                          , msghdr_field(remsg, remsg->to, NULL, term->supports(UTF8))
