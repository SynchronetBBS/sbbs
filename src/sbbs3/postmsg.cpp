...
extern "C" int notify(scfg_t* cfg, uint usernumber, const char* subject, bool strip, const char* text, const char* replyto)
{
	int      i;
	smb_t    smb = {};
	uint16_t xlat;
...
bool sbbs_t::notify(const char* subject, bool strip_ctrl, const char* text, const char* replyto)
{
	char buf[128];

	return ::notify(&cfg, /* usernumber: */ 1, expand_atcodes(subject, buf, sizeof buf), strip_ctrl, text, replyto) == 0;
}
