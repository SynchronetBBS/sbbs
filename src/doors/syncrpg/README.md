...
- **Windows:** MSVC build files are generated by EasyRPG's own CMake
  (`create_project`) the same way as \*nix; a dedicated `build.bat` wrapper
  and MSVC verification are **not yet confirmed on this host** — flag this
  when building on Windows, as with SyncSCUMM.
