...
// SyncConquer door hook: the +/- music-volume hotkeys (door/door_io.c) nudge
// ScoreVolume, since the in-game slider is unreachable with a cell-granular
// (SyncTERM) mouse. Game-neutral name shared with the Red Alert build (its
// definition is in redalert/options.cpp). Tiberian Dawn's ScoreVolume is a
// 0..255 uchar with a single-arg Set_Score_Volume, vs RA's fixed 0..1.
// (SyncConquer PROVENANCE #21.)
extern "C" void SyncConquer_Music_Volume_Step(int dir)
{
    int old = Options.ScoreVolume;             // 0..255
    int v = old + ((dir > 0) ? 26 : -26);      // ~10% of 255
    if (v < 0) {
        v = 0;
    }
    if (v > 255) {
    ...
    if (v == old) {
        return;                                // at the rail -- don't re-apply / rewrite the INI
    }
    Options.Set_Score_Volume(v);
