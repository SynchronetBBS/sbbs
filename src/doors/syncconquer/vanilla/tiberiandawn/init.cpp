...
/* SyncConquer door (door/door_io.c): the resolved -assets game-data dir, added
 * to the file search path at the top of Init_Game (see there). */
...
bool Init_Game(int, char*[])
{
    ...
    /*
    ** SyncConquer door: register the door's game-data (-assets) directory on the
    ** file search path BEFORE any mixfile is opened below. The engine's CWD is
    ** the per-user -home dir (savegames + CONQUER.INI), not where the shared,
    ** read-only MIX data lives -- so without this, every mix (CCLOCAL.MIX,
    ** CONQUER.MIX, ...) fails to open, the MixList stays empty, later
    ** MFCD::Retrieve()s all return NULL, and the first Main_Menu text draw
    ** dereferences a NULL font pointer and crashes. TD's engine never calls
    ** Paths.Init or Parse_Command_Line here (Red Alert's PathsClass DataPath and
    ** the engine's own -CD arg are both dead code in this build), so it reads the
    ** door's already-resolved assets dir directly. Empty (standalone build, or
    ** not yet resolved) => unchanged stock CWD search behavior. PROVENANCE #24.
    */
    {
        const char* datadir = door_engine_data_dir();
        if (datadir != NULL && datadir[0] != '\0') {
            CDFileClass::Add_Search_Drive(datadir);
        }
    }
    ...
    */
    ...
    // SyncConquer door: retrieve these two fonts from the cached MIX (as Red
    // Alert does, redalert/init.cpp) rather than Load_Alloc_Data(CCFileClass(...)).
    // The freeware install ships no LOOSE .fnt files, and a CCFileClass file-open
    // can't read a font out of an already-cached MIX -- it fails "CD not found"
    // and the engine's fatal Prog_End does *((int*)0)=0. MFCD::Retrieve reads the
    // cached mix directly. PROVENANCE #23.
    ...
    if (Get_Resolution_Factor()) {
        // SyncConquer door: MFCD::Retrieve from the cached MIX, not
        // Load_Alloc_Data(CCFileClass(...)) -- see the 6POINT.FNT note above
        // (PROVENANCE #23). These hi-res fonts live in cached mixes; a
        // CCFileClass file-open can't read them and fatals "CD not found".
        MapFontPtr = MFCD::Retrieve("8FAT.FNT");
        Green12FontPtr = MFCD::Retrieve("12GREEN.FNT");
        Green12GradFontPtr = MFCD::Retrieve("12GRNGRD.FNT");
        ScoreFontPtr = MFCD::Retrieve("12GRNGRD.FNT");
        // SyncConquer door: the freeware C&C95 "Gold" data set ships NO 12-point
        // green fonts -- 12GREEN.FNT and 12GRNGRD.FNT are absent from every mix
        // (verified: MFCD::Offset() == 0 for both, with all mixes cached), so the
        // three Retrieves above return NULL and the first in-game score/sidebar
        // text print would dereference a NULL font pointer and crash. Fall back to
        // the 6-point equivalents -- exactly what the low-res branch below already
        // does -- so the game renders (marginally smaller score/sidebar text)
        // instead of crashing. Guarded, so a data set that DOES ship the 12-point
        // fonts keeps using them. PROVENANCE #23.
        if (MapFontPtr == NULL)         MapFontPtr = Font6Ptr;
        if (Green12FontPtr == NULL)     Green12FontPtr = Font3Ptr;
        if (Green12GradFontPtr == NULL) Green12GradFontPtr = GradFont6Ptr;
        if (ScoreFontPtr == NULL)       ScoreFontPtr = GradFont6Ptr;
    } else {
    ...
    */
    // SyncConquer door: retrieve from the cached MIX, not a CCFileClass file-open
    // (the freeware ships no loose TEMPERAT.PAL; CCFileClass fatals "CD not
    // found"). Restores the original MFCD::Retrieve this Vanilla port had
    // commented out. This is only a DEFAULT palette (see the comment above --
    // "only the non terrain specific colors matter"), and TEMPERAT.PAL lives in
    // CONQUER.MIX, which isn't cached until later in this function -- so guard the
    // NULL: GamePalette was allocated MEM_CLEAR (black), a safe default until the
    // scenario loads the real palette. PROVENANCE #23.
    {
        void const* tpal = MFCD::Retrieve("TEMPERAT.PAL");
        if (tpal != NULL) {
            Mem_Copy((void*)tpal, GamePalette, 768L);
        }
    }
...
void Anim_Init(void)
{
    ...
    // SyncConquer door: enable VQA audio decode so FMV cutscenes have dialog --
    // door/vqaaudio_termgfx.cpp ships the decoded audio to the terminal. TD, like
    // RA, shipped this off (the enable below is #if(0)'d / commented). PROVENANCE #22.
...
//	LOCAL (SyncConquer door): the door's own version line, drawn in the menus
//	alongside VersionText (menus.cpp / goptions.cpp) without replacing it. Bump
//	SYNCDAWN_VERSION per release; the date is the SBBS repo's git commit date at
//	build time (common/gitinfo.h) -- Vanilla Conquer is vendored inside that repo,
//	so this stamps the door build. Mirrors redalert/version.cpp's
//	SyncAlert_Version_Name(). PROVENANCE #26.
#define SYNCDAWN_VERSION "v0.1"
char const* SyncDawn_Version_Name(void)
{
    static char buffer[80];

    //	No hash here: this is drawn on the same line as VersionText, which already
    //	carries the (identical) git short-hash -- avoid duplicating it.
    snprintf(buffer, sizeof(buffer), "%s %.10s synchro.net", SYNCDAWN_VERSION, GitCommitDate);
    return (buffer);
}
