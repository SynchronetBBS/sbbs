...
int SoundControlsClass::Init(void)
{
    ...
    MSlider_Height = 8 * factor; //	LOCAL: was 5; taller so a cell-granular (SyncTERM) mouse click lands on it

    FXSlider_X = 147 * factor;
    FXSlider_Y = 40 * factor;
    FXSlider_W = 108 * factor;
    FXSlider_Height = 8 * factor; //	LOCAL: was 5 (see MSlider_Height)
    return (factor);
}
...
void SoundControlsClass::Process(void)
{
    ...
    */
    bool display = true;
    bool process = true;
    int  last_score = Options.ScoreVolume;   // LOCAL: track external (+/- hotkey) volume changes
    ...
    while (process) {
        ...
        **	thumb only updates on menu re-entry. Trigger on a change to
        ...
        **	comparing the two would snap the thumb back mid-drag. Mirrors Red
        **	Alert's sounddlg.cpp fix.
        */
        int score_val = Options.ScoreVolume;
