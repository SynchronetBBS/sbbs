...
void ScrollClass::AI(KeyNumType& input, int x, int y)
{
	...
	*/
    if (!IsRubberBand /*&& !IsTentative*/) {
		...
		** (and consumed) here in the map-input AI chain, ahead of
		** Keyboard_Process(), so it behaves identically in both doors -- and so
		** the arrows don't also drive the stock sidebar-scroll binding.
		*/
        {
            int kdist;
			...
			** Keyboard_Process (the sidebar-scroll binding). On a legacy terminal
			** that only taps (no key-up), the key has already released by now so
			** Down() above missed it -- scroll one full cell here for that case.
			** Home/End jump the view to the map top/bottom.
			*/
            switch (input) {
            case KN_UP:
                if (!Keyboard->Down(KN_UP)) {
