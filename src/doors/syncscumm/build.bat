...
rem   4. Generate git_hash.h, the build stamp the door shows on the GMM.
rem   5. Generate scummvm.sln with the curated engines and OUR backend (no SDL),
rem      the SDL/network/OpenGL features trimmed to a headless door.
rem   6. Wire up vcpkg locally (media libraries) and MSBuild the solution.
rem
...
rem --- 4. The door's build stamp (engines\dialogs.cpp shows it on the GMM) ----
rem Generated by the SHARED helper, exactly as build.sh does it, so a Windows
rem build stamps itself like every sibling door -- including the "~hash" + BUILD
rem time a dirty tree gets, which is the whole point of the shared recipe.
if not exist "%BUILDDIR%" mkdir "%BUILDDIR%"
echo [build] Generating git_hash.h ...
"%CMAKE%" -DOUT="%BUILDDIR%\git_hash.h" -DSRCDIR="%SRCDIR%" ^
  -P "%SRCDIR%\..\..\build\gitinfo.cmake"
if errorlevel 1 goto error

rem --- 5. Generate the ScummVM solution --------------------------------------
...
"%CP%" "..\scummvm" --msvc --termgfx --vcpkg ^
  ...
  --include-dir "%BUILDDIR%" ^
  --library-dir "%BUILDDIR%\lib"
set "GENRC=%errorlevel%"
popd
if not "%GENRC%"=="0" goto error

rem --- 6. Drop the local vcpkg integration next to the solution --------------
...
rem --- 8. Confirm the output and stage it where deploy.js looks --------------
