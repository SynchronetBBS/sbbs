/* syncmoo1_plat.c -- the door's platform seam, implemented over xpdev.
 *
 * See syncmoo1_plat.h for the contract. This is the ONLY translation unit in
 * the door that knows Windows from POSIX, and the only one that pulls in
 * <winsock2.h> (via xpdev's sockwrap.h) -- deliberately, because windows.h's
 * macros collide badly with the vendored 1oom headers.
 *
 ...
 * back to the BBS. That exact bug cost syncalert a release (see commit
 * "syncalert: fix four Windows-only defects that broke the door end to end"),
 * so this is called unconditionally from sm_door_setup() on EVERY launch, not
 * lazily from some failure path. Idempotent, and also called defensively from
 * sm_io_init() in case a future caller reaches the I/O layer first.
 */
...
/* Map the just-failed transfer onto the SM_IO_* codes. SOCKET_ERRNO is xpdev's
 ...
 *
 * ENOBUFS is transient, not fatal: a burst of large sixel frames can exhaust
 * the Winsock buffer pool, and treating that as a dead socket wrongly dropped
 * SyncDuke players back to the BBS (syncduke_io.c says the same). */
static int sm_plat_classify(void)
{
    int e = SOCKET_ERRNO;
...
int sm_plat_redirect_stderr(const char *path)
{
    ...
    if (freopen(path, "w", stderr) == NULL)
        return -1;
    /* Unbuffered so a _exit() (sm_door_hangup) can't drop the buffered tail --
