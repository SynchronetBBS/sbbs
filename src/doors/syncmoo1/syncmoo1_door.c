...
#include "lbxfont.h"        /* 1oom: lbxfont_calc_str_width() -- fit the version line */
 ...
 * SYNCDOOM_VERSION / SYNCDUKE_VERSION / SR_VERSION. The git hash and build
 * date shown next to it come from the build (git_hash.h), not from here. */
#define SYNCMOO1_VERSION "v0.1"
...
/* See syncmoo1.h. Painted by the 1oom main menu (PROVENANCE #4) beside the
 * engine's own "1oom v1.11.8". Kept HERE, not in the vendored uimainmenu.c,
 * so the vendored patch stays a one-line call and all the door identity --
 * the hand-bumped version, the generated hash/date, the fitting rule -- is
 * ours, per CLAUDE.md's ours-vs-vendored contract. */
const char *sm_door_version_line(int maxw)
{
    /* Longest first; each drop sheds the least useful field. GIT_DATE is
     * "Jul 22 2026 23:36" -- a clock time is noise on a version line, so
     * %.11s keeps just the date (same trim as syncretro/main.c). */
    ...
    /* gitinfo.cmake prefixes the SHA with '~' for a dirty tree -- but MOO's LBX
     * fonts have no '~': it is the last slot of their 96-glyph 0x20..0x7e table
     * and is width 0 with zero lit pixels in EVERY font (verified against
     * FONTS.LBX; font 2 has '*' at width 3 / 9 lit). Left as-is the marker
     * draws nothing at all and a dirty build reads exactly like a clean one, so
     * swap in a '*' the font can actually paint. */
    if (hash[0] == '~') {
        snprintf(hbuf, sizeof(hbuf), "*%s", hash + 1);
        hash = hbuf;
    }

    for (i = 0; i < sizeof(form) / sizeof(form[0]); ++i) {
        snprintf(line, sizeof(line), form[i], SYNCMOO1_VERSION, hash, GIT_DATE);
        if (maxw <= 0 || lbxfont_calc_str_width(line) <= maxw)
            break;
    }
    /* Falling out of the loop leaves the shortest form in `line` -- still the
     * best answer when even that overruns. */
    return line;
}
