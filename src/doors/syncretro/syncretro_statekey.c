/* syncretro_statekey.c -- see syncretro_statekey.h.
 *
 * A separate translation unit from syncretro_state.c: sr_state_key() has no
 * caller inside the door (the lobby hands the door its key ready-made), so
 * keeping it out of the door's link avoids pulling md5.c -- and a GCC
 * false-positive -Wstringop-overread on it -- into a binary that never uses
 * either.
 *
