...
int main(int argc, char **argv)
{
	...
	sr_profile_select(sr_door_profile() != NULL ? sr_door_profile() : sr_config_profile(),
	                  core.si.library_name);
...

	{
		...
		for (;;) {
			...
			if (action == SR_DOOR_VOL_UP || action == SR_DOOR_VOL_DOWN) {
				int  pct = sr_audio_volume_step(action == SR_DOOR_VOL_UP ?
				                                +SR_VOLUME_STEP_PCT : -SR_VOLUME_STEP_PCT);
				char msg[64];

				/* At zero the door sends no audio AT ALL, so say off rather
				 * than leave "0%" reading like a stream still costing
				 * bandwidth at an inaudible level. */
				if (pct == 0)
					snprintf(msg, sizeof msg, "Volume 0%% (off)");
				else
					snprintf(msg, sizeof msg, "Volume %d%%", pct);
				sr_io_toast(msg);
			}
	 ...
	 * the session. game_loaded guards the one path that reaches `done:` before a
	 * game ever loaded (rc_core_load_game() failure, above): the core has run
	 * retro_init() but never retro_load_game(), and serializing it then is not a
	 * state libretro promises to support. */
	if (core.game_loaded && sr_door_state_key() != NULL && sr_config_save_dir() != NULL
	    && sr_config_rom_path() != NULL) {
		char path[PATH_MAX];
