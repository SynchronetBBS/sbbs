...
void SetProgVariables(void)
{
	...
	strcpy(od_control.od_prog_name, "Tournament WaHoo V2.0");
	...
	strcpy(ibbsgametitle, "gac_wh");
	strcpy(ibbsid, "wh");
	// Set the validation code for this Game
	strcpy(valcode, "16B4LAZRVL" );
	// Set the executable file name
	#ifdef ODPLAT_NIX
	strcpy(executable, "gac_wh");
	#else
	strcpy(executable, "gac_wh.exe");
	#endif
	return;
}
	...
	// Show the config screen, get user info etc...
	 ...
	 case 'D':
		od_printf("`bright cyan`Enter the Directory to your drop files: `bright white`");
						...
						if (bbsdir[strlen(bbsdir)-1] != '\\') strcat(bbsdir, "\\");
...
void Title( void )
{
	...
	CheckMail();
		...
		od_printf("     `bright white`P`bright blue`)lay the Game\r\n");
			...
			case 'Q':
				...
				if (od_control.user_ansi || od_control.user_rip)
				{
					...
					else
					{
						...
						od_printf("`Bright red`UNREGISTERED!!!`cyan` Please encourage `Bright cyan`%s `cyan` to register!\n\r", od_control.sysop_name);
						od_sleep(3000);
					}
#else
...
/////////////////////////////////////////////////////////////
// does some maintanace for starting the game
void InitGamePlay( void )
{
	...
	if(od_get_key(0))
	{
		...
		od_printf("\r\nYou will need to disable all Ctrl-E (ENQ) answerback "
			...
			"play this game without any problems.\r\n\r\n");
		symbols=0;
		od_get_key(TRUE);
	}
...
void player_write(struct player *pl, FILE *f)
{
	...
	fwrite(pl->locations, sizeof(pl->locations), 1, f);
	fwrite(&(pl->playerColor), sizeof(pl->playerColor), 1, f);
...
void player_read(struct player *pl, FILE *f)
{
	...
	fread(pl->locations, sizeof(pl->locations), 1, f);
	fread(&(pl->playerColor), sizeof(pl->playerColor), 1, f);
