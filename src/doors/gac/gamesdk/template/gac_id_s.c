...
*/
#define EXTDRIVER
#include "\gac_cs\act_0000\gac_fc.h"
#undef EXTDRIVER

void SetProgVariables(void)
{
 
	strcpy(od_control.od_prog_name, "Tournament FreeCell v1.00");
	strcpy(od_control.od_prog_version, "Version 1.00");
	strcpy(od_control.od_prog_copyright, "Copyright 1997 G.A.C. Computer Services");
	strcpy(currentversion, "1.00");
	// ibbsgame title can ONLY be 6 characters long!!!
	strcpy(ibbsgametitle, "GAC_FC");
	strcpy(ibbsid, "FC");
	// Set the validation code for this Game
	strcpy(valcode, "00000000" );
