/* termgfx_plat.c -- termgfx's platform seam, implemented over xpdev.
 *
 * See termgfx_plat.h for the contract. This is the ONLY translation unit in
 * the library that knows Windows from POSIX, and the only one that pulls in
 * <winsock2.h> (via xpdev's sockwrap.h) -- deliberately, because windows.h's
 * macros collide with the termgfx and engine headers every other file
...
int termgfx_plat_sock_setup(int fd)
{
	 ...
	 * setsockopt() calls in termgfx_termio.c (TCP_NODELAY + a large SO_SNDBUF so a
...
/* Map the just-failed transfer onto the TERMGFX_TERMIO_* codes. SOCKET_ERRNO is
...
static int termgfx_plat_classify(void)
{
	int e = SOCKET_ERRNO;

	if (e == EINTR)
		return TERMGFX_IO_INTR;
	if (e == EWOULDBLOCK || e == EAGAIN || e == ENOBUFS)
		return TERMGFX_IO_AGAIN;
	return TERMGFX_IO_ERROR;
}
