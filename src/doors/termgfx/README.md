...
| `apc.{c,h}` | `termgfx_apc_image` | **SyncTERM APC cached-image transport.** Builds the `C;S` *Store* (base64 payload) + `C;Draw…` *Draw* escape sequence that caches an image and blits it. The draw verb is the caller's (`DrawJXL`); SyncTERM also accepts `DrawPPM`, which no door now uses. |
