...
Const
  ...
  '^A^oEnter ^A^n/BJ DOUBLE ^A^oto double down.',
  '^A^oEnter ^A^n/BJ DOUBLE 2 ^A^oto double down on your second hand.',
  '^A^oEnter ^A^n/BJ HAND ^A^oto view the dealer''s and your own cards.',
  '^A^oEnter ^A^n/BJ HELP ^A^ofor this commands list.',
  '^A^oEnter ^A^n/BJ HIT ^A^oto be dealt a card.',
  '^A^oEnter ^A^n/BJ HIT 2 ^A^oto be dealt a card to your second hand.',
  ...
  '^A^oEnter ^A^n/BJ SPLIT ^A^oto split your hand.',
...
begin
  ...
  IDXData.Alias := 'A^pBJ'#8'lackjack Dealer'#0;
  IDXData.Alias[0] := '^';
...
begin
  ...
  TakeNode;
  BroadCastAll (2, '^A^pBJ'#8'lackjack Dealer', '');
...
begin
  ...
  TextAttr := $74;
  GotoXY (1,1);   ClrEol;
  GotoXY (8,1);  Writeln ('Blackjack for TOP'+ProgVerStr+' Copyright 1996 Paul Sidorsky All Rights Reserved.');
...
begin
  ...
  If Length (UsedCards) = 104 then UsedCards := '';
  GetCard := Card;
end;
...
begin
  ...
  begin
    ...
    GiveMSG (9, '', '^A^nFirst hand :    '+ColorHand(NodeHand[Node])+' ^A^n(^A^p'
             +ToStr(HandValue(NodeHand[Node]))+'^A^n)', Node);
    If not NodeDoneH2[Node] then
    GiveMSG (9, '', '^A^nSecond hand:    '+ColorHand(NodeHand2[Node])+' ^A^n(^A^p'
             +ToStr(HandValue(NodeHand2[Node]))+'^A^n)', Node);
  end
  else
    GiveMSG (9, '', '^A^nYour cards:     '+ColorHand(NodeHand[Node])+' ^A^n(^A^p'
             +ToStr(HandValue(NodeHand[Node]))+'^A^n)', Node);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        If NodeStat[NodeData.from] = 2 then
        begin
          Dec (NodeCD[NodeData.from], NodeBet[NodeData.from]*NodeDD[NodeData.from]);
          NextPlayer;
        end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            If (Length (NodeHand[NodeData.from]) = 4) and (NodeHand2[NodeData.from]='') then
            begin
    ...
    begin
      ...
      begin
        If NodeData.from = NodeTurn then
        begin
          If NodeDoneH1[NodeData.from] then GiveMSG (9, '', '^A^nYou can''t play that hand!', NodeData.from)
          else
          begin
            BroadcastBut (9, '', '^A^p'+NodeUser(NodeTurn)+' ^A^nhits.', NodeData.from);
            NodeHand[NodeData.from] := NodeHand[NodeData.from] + GetCard;
            DisplayHand (NodeData.from);
            SecPassed := 0;
            Writeln (NodeUser(NodeTurn), ' hits.');
            If HandValue(NodeHand[NodeData.from]) > 21 then
            begin
              ...
              NodeDoneH1[NodeData.from] := True;
              If NodeHand2[NodeData.from] = '' then NextPlayer
              else If NodeDoneH1[NodeData.from] and NodeDoneH2[NodeData.from] then NextPlayer;
            end;
          end;
        end
        else
    ...
    begin
      If NodeStat[NodeData.from] = 2 then
        ...
        begin
          If (Length (NodeHand[NodeData.from]) = 4) and (NodeHand2[NodeData.from] = '') then
          begin
            If HandValue (NodeHand[NodeData.from][1]+NodeHand[NodeData.from][2])
               = HandValue (NodeHand[NodeData.from][3]+NodeHand[NodeData.from][4]) then
            begin
              GiveMSG (9, '', '^A^nOk, you split.', NodeData.from);
              SecPassed := 0;
              BroadcastBut (9, '', '^A^p'+NodeUser(NodeTurn)+' ^A^nsplits.', NodeData.from);
              NodeHand2[NodeData.from] := NodeHand[NodeData.from][3]+NodeHand[NodeData.from][4];
              Dec (NodeHand[NodeData.from][0], 2);
              NodeHand[NodeData.from] := NodeHand[NodeData.from] + GetCard;
              NodeHand2[NodeData.from] := NodeHand2[NodeData.from] + GetCard;
              DisplayHand (NodeData.from);
            end
        ...
        begin
          ...
          begin
            ...
            begin
              GiveMSG (9, '', '^I^nYou busted!', NodeData.from);
              BroadCastBut (9, '', '^A^p'+NodeUser(NodeTurn)+' ^A^nbusts!', NodeData.from);
              BroadCastBut (9, '', '^A^p'+NodeUser(NodeTurn)+'^I^n loses ^A^p'+ToStr(NodeBet[NodeTurn]*NodeDD[NodeTurn])
                            +' ^A^nCyberdollars!', NodeData.from);
              GiveMSG (9, '', '^A^pYou^I^n lose ^A^p'+ToStr(NodeBet[NodeTurn]*NodeDD[NodeTurn])+' ^A^nCyberdollars!',
                       NodeData.from);
              Dec (NodeCD[NodeData.from], NodeBet[NodeData.from]*NodeDD[NodeData.from]);
              Writeln (NodeUser(NodeTurn), ' busts.');
            end;
            NodeDoneH1[NodeData.from] := True;
            If NodeHand2[NodeData.from] = '' then NextPlayer
            else If NodeDoneH1[NodeData.from] and NodeDoneH2[NodeData.from]
              then NextPlayer;
          end
          else
 ...
 { UsedCards := ''; }
  ...
  For I := 1 to 255 do
    ...
    begin
      ...
      GiveMSG (9, '', '^A^nYour cards:     '+ColorHand(NodeHand[I])+' ^A^n(^A^p'+ToStr(HandValue(NodeHand[I]))+'^A^n)', I);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        BroadCastBut (9, '', '^A^p'+NodeUser(I)+'^A^n''s cards: '+ColorHand(NodeHand[I])+
                   ' ^A^n(^A^p'+ToStr(HandValue(NodeHand[I]))+'^A^n)', I);
        GiveMSG (9, '', '^A^pYour^A^n cards: '+ColorHand(NodeHand[I])+
                   ' ^A^n(^A^p'+ToStr(HandValue(NodeHand[I]))+'^A^n)', I);
      ...
      begin
        ...
        begin
          ...
          GiveMSG (9, '', '^A^pYour^A^n cards: '+ColorHand(NodeHand[I])+
                  ' ^A^n(^A^p'+ToStr(HandValue(NodeHand[I]))+'^A^n)', I);
          BroadCastBut (9, '', '^A^p'+NodeUser(I)+'^I^n loses ^A^p'+ToStr(NodeBet[I]*NodeDD[I])+' ^A^nCyberdollars!',I);
          GiveMSG (9, '', '^A^pYou^I^n lose ^A^p'+ToStr(NodeBet[I]*NodeDD[I])+' ^A^nCyberdollars!',I);
  ...
  begin
    ...
    begin
      ...
      begin
        BroadCastBut (9, '', '^A^p'+NodeUser(I)+'^I^n wins ^A^p'+ToStr(NodeBet[I]*NodeDD[I])+' ^A^nCyberdollars!',I);
        GiveMSG (9, '', '^A^pYou^I^n win ^A^p'+ToStr(NodeBet[I]*NodeDD[I])+' ^A^nCyberdollars!',I);
