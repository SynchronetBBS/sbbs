...
rem             SyncDuke and install syncduke.exe into the door's xtrn dir.
rem
...
rem
rem Mirrors src/doors/syncdoom/build.bat: Visual Studio 2022, classic-mode vcpkg
rem for the static libjxl (JPEG-XL graphics tier).  When the vcpkg prefix is
rem absent the configure step falls back to the sixel/text tiers with a warning -
rem the door still builds.
rem
rem NOTE: Win32 (x86) is the supported target.  The vendored Build engine still
rem has 32-bit assumptions (pointer-as-int32 in the render inner loops), so the
rem x64 build is best-effort.
rem ===========================================================================
...
rem --- Install into xtrn\syncduke -------------------------------------------
set "EXE=%BUILDDIR%\%CONFIG%\syncduke.exe"
if not exist "%EXE%" (
    echo [build] ERROR: expected output not found: %EXE%
    goto error
)
if not exist "%DESTDIR%" mkdir "%DESTDIR%"
copy /Y "%EXE%" "%DESTDIR%\syncduke.exe" >nul
if errorlevel 1 goto error

rem Normalize the destination path for the success message
for %%I in ("%DESTDIR%\syncduke.exe") do set "INSTALLED=%%~fI"
echo.
