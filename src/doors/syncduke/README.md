...
- The **DOOR32.SYS** drop file supplies the client socket and the session time
  limit -- or, with comm type 0, the fact that there is *no* socket and the BBS
  redirected our stdio (see [How the door reaches the
  player](#how-the-door-reaches-the-player)). On a BBS that writes no drop file,
  pass `-s<fd>` and `-t<seconds>` instead.
