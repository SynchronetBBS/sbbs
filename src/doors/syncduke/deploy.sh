#!/bin/sh
# ===========================================================================
# deploy.sh - Install the already-built syncduke binary into the door's xtrn
...
#   Usage:  ./deploy.sh
#
#   Env:    SYNCDUKE_DEST=<dir>    also deploy into this live xtrn/syncduke dir
#                                  (overrides the $SBBSCTRL auto-detect)
#
# Deploys ./build/syncduke to two destinations, each a no-op when it already
# holds the build output:
#   1. THIS tree's ../../../xtrn/syncduke/ -- the in-tree door bundle, next to
#      install-xtrn.ini / syncduke.example.ini.  On the recommended SYMLINK=1
#      *nix install that dir IS the live xtrn (xtrn -> repo/xtrn), so this
#      updates the running door in place.
#   2. The live install located via $SBBSCTRL (root = $SBBSCTRL/..) or
#      $SYNCDUKE_DEST -- for COPY-style installs where the in-tree dir is not
#      the live xtrn.
# The binary is a git-ignored build artifact, so unlike the tracked door files
# it is not carried to a live install by `git pull` / `update.js` -- this script
# is what puts it where the running door reads it.
#
...
# xtrn/syncduke/syncduke can be a server-side symlink back to $EXE, exposed over
...
deploy() {   # $1 = destination xtrn/syncduke directory
	mkdir -p "$1"
	if [ "$EXE" -ef "$1/syncduke" ] || cmp -s "$EXE" "$1/syncduke"; then
		echo "[deploy] $1/syncduke already is the build output -- nothing to copy"
	else
		tmp="$1/.syncduke.tmp.$$"
		cp -f "$EXE" "$tmp" && mv -f "$tmp" "$1/syncduke"
		echo "[deploy] Deployed: $(CDPATH= cd -- "$1" && pwd)/syncduke"
	fi
}
