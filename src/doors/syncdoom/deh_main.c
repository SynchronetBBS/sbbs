...
void DEH_ParseCommandLine(void)
{
    ...
    if (p > 0)
    {
        ...
        while (p < myargc && myargv[p][0] != '-')
        {
            ...
            if (filename != myargv[p])   // syncdoom: this base's D_TryFindWADByName
                free(filename);          // returns the original arg (stack) when not
            ++p;                         // found -- only the found copy is malloc'd
        }
    }
}
