...
// Upstream used SDL_SwapLE32; syncdoom is SDL-free, so convert via the
// endianness i_swap.h detects (identity on little-endian, byte-swap on big).
#ifdef SYS_BIG_ENDIAN
#define cpu_to_le32(x) ((uint32_t) swapLE32(x))
#define le32_to_cpu(x) ((uint32_t) swapLE32(x))
#else
