...
{
    // syncdoom sets net_player_name from the BBS handle (DOOR32.SYS), not from
    // a config file, so there is no config variable to bind here.  (Upstream
    // bound "player_name" via M_BindStringVariable, which doomgeneric's reduced
    // m_config does not provide.)
}
