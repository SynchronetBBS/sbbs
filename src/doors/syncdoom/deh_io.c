...
deh_context_t *DEH_OpenFile(const char *filename)
{
    ...
    fstream = fopen(filename, "r");   // syncdoom: older base has no M_fopen wrapper
...
deh_context_t *DEH_OpenLump(int lumpnum)
{
    ...
    M_StringCopy(context->filename, lumpinfo[lumpnum].name, 9);   // syncdoom: lumpinfo is array-of-structs in our base
