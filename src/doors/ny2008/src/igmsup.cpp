	...
	if (fgets(exenam,250,tparty)!=NULL) {
	  ...
	  if((key=strchr(exenam,'{'))!=0) {
	    ...
	    *(key+x2-x3-1)=0;
	    ...
	    if(od_control.user_security<intval) ok=3;
	  }
	} else {
	  ok=2;
	}
...
//	char *key;
	...
	od_clear_keybuffer();
	od_printf("\n\r\n");
	if (expert!=2 && expert!=3) ny_clr_scr();
...
//	  od_printf("`bright green`? = list  Q = Quit\n\r`bright blue`E`blue`nter `bright blue`Y`blue`er `bright blue`C`blue`ommand (%d mins)`bright blue`>",od_control.caller_timelimit);
	  ...
	  list_igm:;
	  found_file=FALSE;
	  if(od_send_file("other") == TRUE) found_file=TRUE;
	  //od_control.od_ker_exec=NULL;
	  //od_send_file("igm");
	  if(found_file==FALSE)
	    ny_send_menu(OTHER,"");
...
//  ffblk ffblk;
  ...
  INT16 rankt,inact;
	...
	if(exenam2[0]!=0) {
      ...
      if (od_control.baud==0) strcat(exenam," -L");
      //od_control.od_ker_exec=NULL;
      rankt=cur_user.rank;
 ...
 /*	  fgets(cur_user.bbsname,36,justfile);     //the BBS name of the user
	...
	points_raise(p_diff);
      ...
      wrt_sts();
      if(cur_user.alive!=DEAD && cur_user.drug_addiction>=100) Die(1);
      if(cur_user.alive!=DEAD && cur_user.hunger>=100) Die(2);
      if(cur_user.alive!=DEAD && cur_user.std_percent>=100) Die(3);
      ny_kernel();
      //od_control.od_ker_exec=ny_kernel;
      if (cur_user.alive!=ALIVE) od_exit(10,FALSE);
}
