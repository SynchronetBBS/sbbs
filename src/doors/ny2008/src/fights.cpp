...
//	    hit_s*=pow(1.1,user_on->level);
	    ...
	    if (hit_s==0 && en_hit_s==0 && key=='A') goto attack_again_a;
...
//	    hit_s*=pow(1.1,user_on->level);
	    ...
	    if (hit_s==0 && en_hit_s==0 && key=='A') goto attack_again_s;
...
//    system(numstr);
  ...
  do {
    if(fig_m(&erec,user_on,&enm_num,user_num)) return;
    if(o_checks(&enm_num,user_num,user_on,&erec)) return;
/*   do {
...
//	    en_hit_s*=pow(1.1,erec.level);
	    ...
	    if (erec.drug>=COKE) en_hit_s-=xp_random(erec.drug_days_since * pow((double)erec.drug_addiction,1.2))/60;
	    if (en_hit_s<0) en_hit_s=0;
	    ...
	    if (erec.drug>=COKE) en_def_s-=xp_random(erec.drug_days_since * pow(erec.drug_addiction,1.2))/60;
	    if (en_def_s<0) en_def_s=0;
	    ...
	    if (hit_s==0 && en_hit_s==0 && key=='A') goto attack_again_p;
...
//      hit_s*=pow(1.1,user_on->level);
      ...
      if (hit_s==0 && en_hit_s==0 && key=='A') goto attack_again_c;
...
//	  wrt_sts();
  ...
  if (intval<enhitp) intval=enhitp;
  money_plus(intval);
...
{
  if(droga == POT) return (1.0-(high/800.0));
  else if(droga == HASH) return (1.0-(high/650.0));
  else if(droga == LSD) return (1.0-(high/500.0));
  else if(droga == COKE) return ((.5*(high/100.0))+1);
  else if(droga == PCP) return ((high/100.0)+1);
  else return 1;
...
what_drug_force_d(drug_type droga,INT16 high) // Gives the factor for drug in use fer defense
{
  ...
  else if(droga == PCP) return 1;
  else return ((high/100.0)+1);
...
if(zbran == HANDS) return 1.0;
else if(zbran == PEPPER) return 1.5;
else if(zbran == SHARP_STICK) return 1.6;
else if(zbran == SCREWDRIVER) return 1.7;
else if(zbran == KNIFE) return 2.0;
else if(zbran == HAMMER) return 3.0;
else if(zbran == CHAIN) return 4.0;
else if(zbran == LEAD_PIPE) return 5.5;
else if(zbran == GUN) return 7.0;
else if(zbran == COLT) return 8.5;
else if(zbran == RIFLE) return 11.0;
else if(zbran == ELEPHANT_GUN) return 13.3;
else if(zbran == LASER_GUN) return 16.0;
else if(zbran == NAILGUN) return 19.0;
else if(zbran == SHOTGUN) return 25.0;
else if(zbran == ASSAULT_RIFLE) return 33.3;
else if(zbran == MACHINEGUN) return 40.0;
else if(zbran == PROTON_GUN) return 54.0;
else if(zbran == GRANADE_LAUNCHER) return 70.0;
else if(zbran == NEUTRON_PHASER) return 95.0;
else if(zbran == BLASTER) return 120.0;
else if(zbran == ULTRASOUND_GUN) return 127.0;
else return 1.0;
...
//    od_printf("`bright red`T`red`hey give ya hit of yer favorite drug ...\n\r");
    ...
    if (user_on->drug_hits<32767) user_on->drug_hits++;
...
//    od_printf("`bright red`Y`red`a fell inside a hole ...\n\r");
  ...
  if(intval==6) {
    ...
    if((intval + user_on->rocks) > 250) intval=250 - user_on->rocks;
    ny_line(183,0,2);
    //Stuff happens...
    if(rip)
      od_get_answer("\n\r");
...
//  FILE *phile;
  char *keyp;
    ...
    if(cnt2>0) fgets(string,300,phile);
...
//  line++;
  ...
  if(cnt2>0) fgets(lines,300,phile);
  for(cnt=0;cnt<line;cnt++)
