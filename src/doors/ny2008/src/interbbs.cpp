...
void EncodeMail(char *pszDest,ibbs_mail_type *ibmail)
{
   ...
   *pcDest='\n';
   ...
   for(x=0;x<l;x++) {
     sprintf(pcDest,"%X",(INT16)ibmail->node_s[x]);
     pcDest+=2;
   }
   *pcDest='\n';
   pcDest++;
   ...
   for(x=0;x<l;x++) {
     sprintf(pcDest,"%X",(INT16)ibmail->node_r[x]);
     pcDest+=2;
   }
   *pcDest='\n';
   pcDest++;
   ...
   for(x=0;x<l;x++) {
     sprintf(pcDest,"%X",(INT16)ibmail->recver[x]);
     pcDest+=2;
   }
   *pcDest='\n';
   pcDest++;
   ...
   for(x=0;x<l;x++) {
     sprintf(pcDest,"%X",(INT16)ibmail->recverI[x]);
     pcDest+=2;
   }
   *pcDest='\n';
   pcDest++;
   ...
   *pcDest='\n';
   ...
   for(ln=0;ln<(ibmail->length + ibmail->quote_length) && ln<20;ln++) {
     ...
     for(x=0;x<l && x<20 ;x++) {
       sprintf(pcDest,"%X",(INT16)ibmail->lines[ln][x]);
       pcDest+=2;
     }
...
INT16 DecodeBufferR(const char *pszSource, void *pDestBuffer)
   {
   const char *pcSource = pszSource;
   char *pcDest = (char *)pDestBuffer;
   char *size[2];
