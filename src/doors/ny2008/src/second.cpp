...
//	ffblk ffblk;
	...
	do {
	  ...
	  if (key=='T') justfile=ShareFileOpen(TODNEWS_FILENAME,"rb");
	  else justfile=ShareFileOpen(YESNEWS_FILENAME,"rb");
...
//	        WaitForEnter();
	  ...
	  if(rip) od_send_file("frame2.rip");
	  ny_line(65,1,0);
...
guns_ops(void)
{
	...
	do {
	  ...
	  if (key=='Y') {
	    ...
	    if (s_key!='Q') {
	      if (s_key=='A') choice=PEPPER;
	      else if (s_key=='B') choice=SHARP_STICK;
	      else if (s_key=='C') choice=SCREWDRIVER;
	      else if (s_key=='D') choice=KNIFE;
	      else if (s_key=='E') choice=HAMMER;
	      else if (s_key=='F') choice=CHAIN;
	      else if (s_key=='G') choice=LEAD_PIPE;
	      else if (s_key=='H') choice=GUN;
	      else if (s_key=='I') choice=COLT;
	      else if (s_key=='J') choice=RIFLE;
	      else if (s_key=='K') choice=ELEPHANT_GUN;
	      else if (s_key=='L') choice=LASER_GUN;
	      else if (s_key=='M') choice=NAILGUN;
	      else if (s_key=='N') choice=SHOTGUN;
	      else if (s_key=='O') choice=ASSAULT_RIFLE;
	      else if (s_key=='P') choice=MACHINEGUN;
	      else if (s_key=='R') choice=PROTON_GUN;
	      else if (s_key=='S') choice=GRANADE_LAUNCHER;
	      else if (s_key=='T') choice=NEUTRON_PHASER;
	      else if (s_key=='U') choice=BLASTER;
	      else if (s_key=='V') choice=ULTRASOUND_GUN;
	      else if (s_key=='W') {
		choice=A_BOMB;
...
take_drug(void)
{
  ...
  if (cur_user.drug_hits<=0) {   // check if got some drugs
    ...
    if (cur_user.drug_high>100) cur_user.drug_high=100;
    ny_line(83,0,0);
    //You are now
...
//		    `green` condoms.\n\r\n
		    ...
		    if (longval+cur_user.condoms>INT_MAX) longval=INT_MAX-cur_user.condoms;
		    ny_line(141,0,0);
...
//  WaitForEnter();
  ...
  if(ill>AIDS) ill=AIDS;
  if(cur_user.std>AIDS) cur_user.std=AIDS;
  if (ill>=cur_user.std && ill!=NONE) {
    cur_user.std=ill;
	     ...
	     quant;
			...
			do {
			  ...
			  if (s_key=='Y') {
			    ...
			    if ((max+cur_user.drug_hits)>INT_MAX) max=INT_MAX-cur_user.drug_hits;
			    if ((cur_user.drug_hits+max)>max_drug_hits) max=max_drug_hits-cur_user.drug_hits;
			    if(rip) {
			      od_send_file("input.rip");
			    }
			    ny_line(158,0,0);
...
/*			      if (cur_user.drug!=POT) od_printf("1`green`. `bright green`P`green`ot\n\r");
				...
				if (f_key!='N') {
				  ...
				  if (t_key=='1') print_drug(cur_user.drug=POT);
				  else if (t_key=='2') print_drug(cur_user.drug=HASH);
				  else if (t_key=='3') print_drug(cur_user.drug=LSD);
				  else if (t_key=='4') print_drug(cur_user.drug=COKE);
				  else if (t_key=='5') print_drug(cur_user.drug=PCP);
				  else if (t_key=='6') print_drug(cur_user.drug=HEROIN);
...
//      od_printf("\n\r\n");
	...
	ny_line(429,0,1);
	if (cur_user.drug_hits<32767) cur_user.drug_hits++;
    ...
    ny_line(432,0,1);
    if (cur_user.condoms<32767) cur_user.condoms++;
    if(!rip)
      WaitForEnter();
    else
      od_get_answer("\n\r");
...
//	strset(input_s,' ');
	...
	cnt=0;
	while (1) {
	  if (cnt<29) cnt++; else od_printf("\b");
	  ...
	  if(input_s[cnt]=='>') {
	    return max;
	     ...
	     if (intval>max) {
	     do {od_printf("\b \b"); cnt--;} while (cnt>0);
	     intval=max;
	     od_printf("%lu",max);}
	     od_printf("\n\r");
	     break;
...
//
	  ...
	  if (key=='M') cur_user.sex = MALE; else cur_user.sex = FEMALE;

	  if(!rip) ny_send_menu(NATION,"");
/*	  od_printf("\n\r\n`bright red`W`red`hat are you....\n\r\n");
	  ...
	  switch(key) {
	    case '1':
	      cur_user.nation = HEADBANGER; break;
	    case '2':
	      cur_user.nation = HIPPIE; break;
	    case '3':
	      cur_user.nation = BIG_FAT_DUDE; break;
	    case '4':
	      cur_user.nation = CRACK_ADDICT; break;
	    case '5':
	      cur_user.nation = PUNK; break;
	  }
...
void CustomConfigFunction(char *pszKeyword, char *pszOptions)
{
  if(stricmp(pszKeyword, "BustedChanceBank") == 0)
  {
    ...
    if (busted_ch_bank<1) busted_ch_bank=1;
    if (busted_ch_bank>90) busted_ch_bank=90;
  }
  else if(stricmp(pszKeyword, "BustedChanceFood") == 0)
  {
    sscanf(pszOptions,"%d",&busted_ch_food);
    if (busted_ch_food<1) busted_ch_food=1;
    if (busted_ch_food>90) busted_ch_food=90;
  }
  else if(stricmp(pszKeyword, "BustedChanceRape") == 0)
  {
    sscanf(pszOptions,"%d",&busted_ch_rape);
    if (busted_ch_rape<1) busted_ch_rape=1;
    if (busted_ch_rape>90) busted_ch_rape=90;
  }
  else if(stricmp(pszKeyword, "BustedChanceBeggar") == 0)
  {
    sscanf(pszOptions,"%d",&busted_ch_beggar);
    if (busted_ch_beggar<1) busted_ch_beggar=1;
    if (busted_ch_beggar>90) busted_ch_beggar=90;
  }
  else if(stricmp(pszKeyword, "BustedChanceCar") == 0)
  {
    sscanf(pszOptions,"%d",&busted_ch_car);
    if (busted_ch_car<1) busted_ch_car=1;
    if (busted_ch_car>90) busted_ch_car=90;
  }
  else if(stricmp(pszKeyword, "BustedChanceSchool") == 0)
  {
    sscanf(pszOptions,"%d",&busted_ch_school);
    if (busted_ch_school<1) busted_ch_school=1;
    if (busted_ch_school>90) busted_ch_school=90;
  }
  else if(stricmp(pszKeyword, "BustedChanceWindow") == 0)
  {
    sscanf(pszOptions,"%d",&busted_ch_window);
    if (busted_ch_window<1) busted_ch_window=1;
    if (busted_ch_window>90) busted_ch_window=90;
  }
  else if(stricmp(pszKeyword, "BustedChancePoison") == 0)
  {
    sscanf(pszOptions,"%d",&busted_ch_poison);
    if (busted_ch_poison<1) busted_ch_poison=1;
    if (busted_ch_poison>90) busted_ch_poison=90;
  }
  else if(stricmp(pszKeyword, "BustedChanceBomb") == 0)
  {
    sscanf(pszOptions,"%d",&busted_ch_bomb);
    if (busted_ch_bomb<1) busted_ch_bomb=1;
    if (busted_ch_bomb>90) busted_ch_bomb=90;
  }
  else if(stricmp(pszKeyword, "SuccessChanceBank") == 0)
  {
    sscanf(pszOptions,"%d",&success_ch_bank);
    if (success_ch_bank<1) success_ch_bank=1;
    if (success_ch_bank>100) success_ch_bank=100;
  }
  else if(stricmp(pszKeyword, "SuccessChanceFood") == 0)
  {
    sscanf(pszOptions,"%d",&success_ch_food);
    if (success_ch_food<1) success_ch_food=1;
    if (success_ch_food>100) success_ch_food=100;
  }
  else if(stricmp(pszKeyword, "SuccessChanceRape") == 0)
  {
    sscanf(pszOptions,"%d",&success_ch_rape);
    if (success_ch_rape<1) success_ch_rape=1;
    if (success_ch_rape>100) success_ch_rape=100;
  }
  else if(stricmp(pszKeyword, "SuccessChanceBeggar") == 0)
  {
    sscanf(pszOptions,"%d",&success_ch_beggar);
    if (success_ch_beggar<1) success_ch_beggar=1;
    if (success_ch_beggar>100) success_ch_beggar=100;
  }
  else if(stricmp(pszKeyword, "SuccessChanceCar") == 0)
  {
    sscanf(pszOptions,"%d",&success_ch_car);
    if (success_ch_car<1) success_ch_car=1;
    if (success_ch_car>100) success_ch_car=100;
  }
  else if(stricmp(pszKeyword, "SuccessChanceSchool") == 0)
  {
    sscanf(pszOptions,"%d",&success_ch_school);
    if (success_ch_school<1) success_ch_school=1;
    if (success_ch_school>100) success_ch_school=100;
  }
  else if(stricmp(pszKeyword, "SuccessChanceWindow") == 0)
  {
    sscanf(pszOptions,"%d",&success_ch_window);
    if (success_ch_window<1) success_ch_window=1;
    if (success_ch_window>100) success_ch_window=100;
  }
  else if(stricmp(pszKeyword, "SuccessChancePoison") == 0)
  {
    sscanf(pszOptions,"%d",&success_ch_poison);
    if (success_ch_poison<1) success_ch_poison=1;
    if (success_ch_poison>100) success_ch_poison=100;
  }
  else if(stricmp(pszKeyword, "SuccessChanceBomb") == 0)
  {
    sscanf(pszOptions,"%d",&success_ch_bomb);
    if (success_ch_bomb<1) success_ch_bomb=1;
    if (success_ch_bomb>100) success_ch_bomb=100;
  }
  else if(stricmp(pszKeyword, "FightsPerDay") == 0)
  {
    sscanf(pszOptions,"%d",&max_fights);
    if (max_fights<0) max_fights=0;
    if (max_fights>800) max_fights=800;
  }
  ...
  else if(stricmp(pszKeyword, "DeleteAfter") == 0)
  {
    ...
    if (delete_after<5) delete_after=5;
    if (delete_after>800) delete_after=800;
  }
  else if(stricmp(pszKeyword, "PollingValue") == 0)
  {
    sscanf(pszOptions,"%d",&time_slice_value);
    if (time_slice_value<0) time_slice_value=0;
    if (time_slice_value>2000) time_slice_value=2000;
  }
  else if(stricmp(pszKeyword, "CheckFlagsEvery") == 0)
  {
    sscanf(pszOptions,"%d",&check_o_nodes);
    if (check_o_nodes<1) check_o_nodes=1;
    if (check_o_nodes>60) check_o_nodes=60;
  }
    else if(stricmp(pszKeyword, "BankInterest") == 0)
  {
    sscanf(pszOptions,"%d",&bank_interest);
    if (bank_interest<0) bank_interest=0;
    if (bank_interest>100) bank_interest=100;
  }
  ...
  *pn=-1;
  ...
  while(ny_fread(&ibscr_rec,sizeof(ibbs_scr_rec),1,justfile)==1) {
	...
	if(ibscr_rec.level>=0) {
	  ...
	  if (ibscr_rec.sex==MALE) ny_disp_emu("`$M  "); else ny_disp_emu("`$F  ");
	  od_set_attrib(0x09);
	  switch(ibscr_rec.nation) {
	    case HEADBANGER:
	      od_printf("HEADBANGER   "); break;
	    case HIPPIE:
	      od_printf("HIPPIE       "); break;
	    case BIG_FAT_DUDE:
	      od_printf("BIG FAT DUDE "); break;
	    case CRACK_ADDICT:
	      od_printf("CRACK ADDICT "); break;
	    case PUNK:
	      od_printf("PUNK         "); break;
	  }
	} else {
	  ny_disp_emu("`@     ");
	  ny_disp_emu(ibscr_rec.name,25);
...
//	          od_printf("\b\b`bright red`S`red`aving...\n\r");
		  ...
		  ny_line(136,0,1);
		  if(ibmail.flirt==1) cur_user.sex_today++;
		}
...
//	              od_printf("\b\b`bright red`S`red`aving...\n\r");
		...
		if(money>cur_user.money) {
		  ...
		  *(INT16 *)ibmail.lines[0]=intval-1;
		  if(intval==1) strcpy(ibmail.lines[1],"George");
		  else if(intval==2) strcpy(ibmail.lines[1],"Gang Member");
		  else if(intval==3) strcpy(ibmail.lines[1],"Mafia Guy");
		  else if(intval==4) strcpy(ibmail.lines[1],"Lloyd The Bloody");
		  else if(intval==5) strcpy(ibmail.lines[1],"David Koresh");
		  else if(intval==6) strcpy(ibmail.lines[1],"Masked Fella");
		  intval=ibbs_bbs_list();
		  if(intval>-1) {
		    ibbs_bbs_name(intval,0,FALSE,hand,&dbn,&pn);
		    if(hand[0]!=0) {
