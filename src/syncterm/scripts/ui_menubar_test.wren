...
class UiMenubarTest {
  ...
  static makeBar_(fired) {
    var bar = MenuBar.new()
    bar.bounds = Rect.new(1, 1, 30, 1)
    bar.items = [
      ["File", Fn.new { fired[0] = "File" }],
      ["Edit", Fn.new { fired[0] = "Edit" }],
      ["Help", Fn.new { fired[0] = "Help" }],
    ]
    return bar
  }
  ...
  static testHotkeyActivates_() {
    ...
    check_(fired[0] == "Edit" && bar.focusedItem == 1,
  ...
  static testFocusedItemHasDifferentStyle_() {
    var fired = [null]
    var a = makeBar_(fired)
    var b = makeBar_(fired)
