// Shared Yankee Trader control-panel UI. The connected entry point registers
// terminal hooks; the trusted main-menu extension invokes its offline mode.
...
import "yankee_trader_guide" for YTGuide
import "yankee_trader_state" for YTState
import "yankee_trader_tools" for YTCommands, YTText, YTUniverse

class YankeeTraderUi {
  static initialize_() {
    __offline = false
    __offlineBbsName = null
    __offlineTimestamp = null
  ...
  static run() {
    ...
    Fiber.new {
      var action = null
      uiRun_(Fn.new { action = mainLoop_() })
      Input.setupMouseEvents()
      if (action != null) action.call()
    }.call()
  }

  static runOffline(bbsName, timestamp) {
    initialize_()
    __offline = true
    __offlineBbsName = bbsName
    __offlineTimestamp = timestamp
    uiRun_(Fn.new { offlineMainLoop_() })
    __offline = false
    __offlineBbsName = null
    __offlineTimestamp = null
  }

  // Connected sessions must suspend CTerm while a child UI owns the screen.
  // The menu VM has no active CTerm and deliberately denies that capability,
  // so its equivalent is only a screen snapshot around the synchronous app.
  static uiRun_(fn) {
    if (!__offline) return Screen.modalRun(fn)
  ...
  static offlineMainLoop_() {
    var selected = 0
    while (true) {
      var rows = [
        ["calculators", "Calculators"],
        ["universe", "Universe maps and command planning"],
        ["dashboard", "Dashboard"],
        ["macros", "F1-F12 key-binding reference"],
        ["games", "Games and settings"],
        ["strategy", "Strategy reference"],
        ["records", "Records and notes"]
      ]
      var picked = pickWithSelection_(
          "Yankee Trader Offline: %(YTState.activeGameName)",
          offlineHelp_, rows.map {|row| row[1] }.toList, selected)
      if (picked < 0) return
      selected = picked
      var action = rows[picked][0]
      if (action == "calculators") calculators_()
      if (action == "universe") universe_()
      if (action == "dashboard") dashboard_()
      if (action == "macros") macros_()
      if (action == "games") profile_()
      if (action == "strategy") reference_()
      if (action == "records") records_()
    }
  }
  ...
  static universe_() {
    ...
    while (true) {
      var rows = [
        ...
        [2, __offline ? "Plan saved-port reports" : "Scan saved ports"],
        [3, __offline ? "Plan actions from saved C;10 analysis" :
            "Act on saved C;10 analysis"],
        [4, __offline ? "Plan actions for a sector range" :
            "Scan a sector range"],
        [5, __offline ? "Plan navigation avoid changes" :
            "Manage navigation avoids"],
        [8, "Legacy and recovery imports"]
      ]
      if (YTState.data["profile"]["mercenaries"]) {
        rows.add([6, __offline ? "Plan a mercenary fighter surround" :
            "Surround mercenaries with fighters"])
      }
      if (!__offline && commandRunStatus_["active"]) {
  ...
  static analyzeLog_() {
    ...
    var error = YTState.mergeGraph(graph, currentDate_)
    if (error != null) {
  ...
  static reviewCommandPlan_(title, description, warning, commands, itemCount) {
    ...
    while (true) {
      ...
      var picked = pickWithSelection_("%(title): Review", help, choices,
          selected)
      if (picked < 0 || picked == choices.count - 1) return null
      selected = picked
      if (!__offline && picked == 0) {
        if (confirm_(title,
  ...
  static commandPlanHelp_(title, description, warning, itemCount, batchCount) {
    var runHelp = __offline ?
        "**Offline safety** keeps every command local. Preview or copy the " +
        "plan for a later connected session; this panel cannot send it.\n\n" :
  ...
  static records_() {
    ...
    while (true) {
      var picked = pickWithSelection_(
        ...
        __offline ? "Plan planet/fighter reconciliation reports" :
  ...
  static profile_() {
    ...
    while (true) {
      var p = YTState.data["profile"]
      var rows = [
        ["switch", "Switch game (%(YTState.games.count) configured)"],
        ["add", "Add another game"],
        ["rename", "Rename active game"],
        ["delete", "Delete active game"]
      ]
      if (!__offline) {
        rows.add(["detect", "Detect game settings from the connected door"])
      }
      rows.add(["defaults", "Apply version defaults: %(p["version"])"])
      rows.add(["maxSector", "Universe size: %(p["maxSector"])"])
      rows.add(["turnsPerDay", "Turns per day: %(p["turnsPerDay"])"])
      rows.add(["maxPlanets", "Maximum planets: %(p["maxPlanets"])"])
      rows.add(["maxPorts", "Maximum ports: %(p["maxPorts"])"])
      rows.add(["sectorAvoidSlots",
          "Sector avoid slots: %(p["sectorAvoidSlots"])"])
      rows.add(["macroRepeats", "Macro repeat count: %(p["macroRepeats"])"])
      rows.add(["missiles", "Missiles: %(enabled_(p["missiles"]))"])
      rows.add(["plasmaBolts",
          "Plasma bolts: %(enabled_(p["plasmaBolts"]))"])
      rows.add(["xannor", "Xannor: %(enabled_(p["xannor"]))"])
      rows.add(["mercenaries",
          "Mercenaries: %(enabled_(p["mercenaries"]))"])
      rows.add(["sensorRobots",
          "Sensor robots: %(enabled_(p["sensorRobots"]))"])
      rows.add(["dangerScanner",
          "Danger scanner: %(enabled_(p["dangerScanner"]))"])
      rows.add(["spies", "Spies: %(enabled_(p["spies"]))"])
      rows.add(["export", "Copy all BBS games as WON"])
      rows.add(["import", "Replace all BBS games from clipboard WON"])
      var picked = pickWithSelection_(
      ...
      selected = picked
      var action = rows[picked][0]
      if (action == "switch") switchGame_()
      if (action == "add") addGame_()
      if (action == "rename") {
      ...
      var flags = ["missiles", "plasmaBolts", "xannor", "mercenaries",
          "sensorRobots", "dangerScanner", "spies"]
      if (flags.indexOf(action) >= 0) {
        reportSave_(YTState.setProfileOption(action, !p[action]))
      }
      if (action == "export") {
        Clipboard.text = YTState.exportText
        alert_("State Export", "All games for this BBS are on the clipboard.")
      }
      if (action == "import") importState_()
    }
  }
  ...
  static reference_() {
    var topics = strategyTopics_
    var selected = 0
    while (true) {
      var picked = pickWithSelection_("Strategy Reference",
          strategyReferenceHelp_, topics.map {|t| t[0] }.toList, selected)
      if (picked < 0) return
      selected = picked
      guide_(topics[picked][0], topics[picked][1])
    }
  }

  static strategyTopics_ {
    return YTGuide.topics(YTState.activeGameName, YTState.data["profile"])
  }
  ...
  static onSensor_(match) {
    ...
    var saveError = null
    if (graph.count > 0) saveError = YTState.mergeGraph(graph, currentDate_)
  ...
  // intentionally leave their final frame visible; uiRun_ restores the
  // menu or terminal content that the child covered.
  static pick_(title, helpText, items) {
    return uiRun_(Fn.new {
      return __offline ?
          ListPicker.pickSync(title, helpText, items) :
          ListPicker.pick(title, helpText, items)
    })
  }
  ...
  static pickWithSelection_(title, helpText, items, selected) {
    return uiRun_(Fn.new {
      return __offline ?
          ListPicker.pickWithSelectionSync(title, helpText, items, selected) :
          ListPicker.pickWithSelection(title, helpText, items, selected)
    })
  }
  ...
  static alert_(title, message) {
    uiRun_(Fn.new { MenuUi.alertStandalone(title, message) })
  }

  static guide_(title, body) {
    uiRun_(Fn.new {
      var app = App.new()
      var viewer = Help.new(title, body)
      viewer.bounds = Help.centeredBounds_()
      viewer.onDismiss = Fn.new {|value| app.quit() }
      app.pushModal(viewer)
      app.runSync()
    })
  }
  ...
  static hookConfirm_(title, message) {
    ...
    popup.title = title
    popup.bounds = Popup.centeredTitledBounds_(title, message, 1, 24)
  ...
  static currentTimestamp_ {
    return __offline ? __offlineTimestamp : BBS.connected + Conn.elapsedSeconds
  }

  static currentDate_ { CalendarDate.isoFromTimestamp(currentTimestamp_) }

  static date_(title, message) {
    var result = null
    var today = CalendarDate.fromTimestamp(currentTimestamp_)
    uiRun_(Fn.new {
      var app = App.new()
      var pane = Pane.new()
      pane.title = message
  ...
  static offlineHelp_ { "# Yankee Trader Offline Planner\n\n" +
      "System: %(__offlineBbsName)\nGame: %(YTState.activeGameName)\n\n" +
      "This panel opens the selected BBS entry's saved Yankee Trader data " +
      "without making a connection.\n\n" +
      "**Calculators** provides the same live-updating combat, economy, " +
      "planet, Xannor, mercenary, and trading forms as the connected panel.\n\n" +
      "**Universe maps and command planning** explores saved C;10 paths and " +
      "builds sector, port, avoid, missile, robot, and reconciliation command " +
      "batches. Offline plans can be previewed or copied; they are never sent.\n\n" +
      "**Dashboard** summarizes the selected game's profile, saved map, " +
      "records, and prior benchmark results.\n\n" +
      "**F1-F12 key-binding reference** explains the commands that will be " +
      "available the next time this BBS is connected.\n\n" +
      "**Games and settings** maintains the BBS's saved profiles. Live game " +
      "detection is intentionally available only while connected.\n\n" +
      "**Strategy reference** opens the detailed topic guide.\n\n" +
      "**Records and notes** reviews or updates sectors, planets, port pairs, " +
      "and intelligence before the next session." }
  ...
  static universeHelp_ {
    var execution = __offline ?
        "Offline command plans can be previewed or copied for a later " +
        "session; this panel never sends them." :
        "Actions that contact the door open a review screen before sending " +
        "or copying commands."
    var cancel = __offline ? "" :
        "**Cancel running command plan** stops later batches of the active " +
        "Universe or Records action.\n\n"
    return "# Universe Mapping and Actions\n\n" +
  ...
  static dataHelp_ {
    var detector = __offline ?
        "**Detect game settings** is connection-dependent and therefore " +
        "appears only in the connected Alt-Y panel." :
        "**Detect game settings** reads Info, Version, Help, and Computer " +
        "screens, finds the sector ceiling with C;2 queries, and presents " +
        "every detected or inferred value for review."
    return "# Games and Settings\n\n" +
  ...
  static gameHelp_ { "# Select Game\n\nChoose the Yankee Trader game whose " +
      "data you want to use. The selected profile supplies sector limits, " +
  ...
  static strategyReferenceHelp_ { "# Strategy Reference\n\n" +
      "This is a detailed, topic-by-topic adaptation of Zharous's strategy " +
      "guide. Select a topic to open a scrollable article; use Up/Down or " +
      "Page Up/Page Down to read it.\n\n" +
      "**Economy and planets** covers opening priorities, exact trade loops, " +
      "production breakpoints, bank effects, placement, movement, and capture.\n\n" +
      "**Combat and secrecy** covers weapon formulas, retaliation, surrender, " +
      "cloak, newspaper exposure, and online-player quirks.\n\n" +
      "**Xannor and mercenaries** covers score-to-defense math, turn farming, " +
      "Xannoron, bribes, surrounds, and the Mercenary Base.\n\n" +
      "**Mapping and intelligence** covers C;10 paths, dead ends, C;2 and C;9 " +
      "probing, missiles, robots, spies, planet-bank inference, and Xannor HQ " +
      "backtracing.\n\n" +
      "**Version and balance topics** separate stock 3.6 mechanics from YT 3.2 " +
      "and describe common patches that can invalidate a tactic." }
