...
#define MultW 2
	...
	switch ((char)codeStr[0]) {
		...
		case 'o':
			...
			cterm_write(cterm, "~~~~~~~~~~~~\r\n", -1, NULL, 0, NULL);
			term_setattr(WHITE);
			cterm_write(cterm, "Suit : ", -1, NULL, 0, NULL);
		...
		case 't':
			...
			term_setattr(LIGHTCYAN);
			cterm_write(cterm, "-[H]  ", -1, NULL, 0, NULL);
			term_setattr(CYAN);
			...
			if (!zz) {
				term_setattr(LIGHTCYAN);
				cterm_write(cterm, "-[R]  ", -1, NULL, 0, NULL);
				term_setattr(CYAN);
			...
			term_setattr(LIGHTCYAN);
			cterm_write(cterm, "-[V]  ", -1, NULL, 0, NULL);
			term_setattr(MAGENTA);
			cterm_write(cterm, "View Opponent\r\n\r\n", -1, NULL, 0, NULL);
			term_setattr(LIGHTCYAN);
			cterm_write(cterm, "-[M]  ", -1, NULL, 0, NULL);
			term_setattr(LIGHTGRAY);
		...
		case 'u':
			...
			term_setattr(WHITE);
			cterm_write(cterm, "Camp :[ ", -1, NULL, 0, NULL);
			term_setattr(LIGHTGREEN);
			cterm_write(cterm, menuBlock, -1, NULL, 0, NULL);
			term_setattr(WHITE);
			cterm_write(cterm, " ]:\r\n", -1, NULL, 0, NULL);
...
incomingMapScanner(unsigned char *codeStr)
{
	...
	switch ((char)codeStr[0]) {
		case 'F':
			...
			cterm_write(cterm, "[ ]  ", -1, NULL, 0, NULL);
			term_setattr(13);
			cterm_write(cterm, "Bases         Condition  ", -1, NULL, 0, NULL);
			term_setattr(5);
			cterm_write(cterm, "[:            ", -1, NULL, 0, NULL);
			term_setattr(1);
			cterm_write(cterm, "³\r\n", -1, NULL, 0, NULL);
			...
			for (zz = 1; zz < 9; zz++) {
				switch (zz) {
					...
					case 8:
						...
						switch (yy) {
							case 0:
								cterm_write(cterm, "Not Hungry", -1, NULL, 0, NULL);
								break;
							case 1:
								cterm_write(cterm, "Bit Hungry", -1, NULL, 0, NULL);
								break;
							case 2:
								cterm_write(cterm, "Hungry    ", -1, NULL, 0, NULL);
								break;
							case 3:
								term_setattr(12);
								cterm_write(cterm, "HUNGRY!   ", -1, NULL, 0, NULL);
							...
							case 4:
								term_setattr(12 + 128);
								cterm_write(cterm, "STARVING!!", -1, NULL, 0, NULL);
								break;
						}
						break;
				}
