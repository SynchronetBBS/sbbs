...
| `classic_theme`
| library
| Maps the six Classic Theme settings to Wren `Theme` roles.  The shipped
  menu and file picker use it; connected interfaces continue to use their
  own themes, including `Theme.default` unless changed by script.
...
and mouse pushback, clears input claims and waiters owned by the screen
being left, and rejects late input-shaped results tagged with an older
epoch.  This prevents a connected script from placing synthetic input in
...
| Classic Theme palette indexes used by the shipped Wren main menu and
  picker.  Frame, text, and lightbar foreground use `Menu.colors`,
  ...
  The shared `classic_theme.wren` module resolves those sentinels and builds
  an App `Theme`.  Custom menu and picker overrides must apply the values
  themselves if they want to preserve that preference.  Connected-session
  interfaces do not receive this theme automatically.
|===
...
each wheel notch moves the selection one
...
lightbar selection, gray-on-blue scrollbar, with a classic inactive
