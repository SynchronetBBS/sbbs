...
active editor is left-aligned within the middle `screen width - 4` cells.
...
positioning preserves the value and switches to the normal menu colors for
ordinary editing.  Leaving the editor restores the centered inactive row
immediately, even when its value was unchanged.
...
mode, matching SyncTERM's reversed UIFC cursor setting.  `cursorAttr` supplies
