...
the order shown by that returned List.
...
| Fetch the URI into the web-list cache, insert the entry, and persist
  the `WebLists` INI section.  Returns `null` on success or an error
  String.  Persistence failure rolls the in-memory insertion back.

| `updateWebList(index, uri)`
| Change and persist an entry's URI.  Returns Bool and restores the old
  URI if persistence fails.  It does not refresh the cached list.

| `deleteWebList(index)`
| Remove and persist an entry.  Returns Bool and restores the entry at
  its original index if persistence fails.
...
Add, update, and delete are disabled in safe mode before any fetch or
in-memory mutation.  Refresh remains a read/fetch operation and is
permitted.  No web-list method exposes its cache pathname as a file
capability.
...
| User-consent escape hatch for "upload from anywhere": invokes
  ...
  `File` is read-only and carries a `.token` consent token (see
  "Picker consent tokens" below) when the per-installation signing
  key is loaded.
