...
`Hook.onKey` handler being the canonical case — set up the App
yourself and wire `Popup.onDismiss=` to quit it:

[source,wren]
----
var app = App.new()
var c   = Confirm.new("Disconnect... Are you sure?")
c.bounds    = Popup.centeredBounds_(c.message, 1, 24)
...
`onDismiss=(fn)` fires from `dismissWith_` after the popup pops
itself off the modal stack, with the dismissed value (Bool / String
/ null depending on the popup subclass).  Without this wire-up
