...
| `keys_default`
| auto / connected
| Default `Hook.onKey` bindings for the in-terminal hot-keys that
  historically lived as `case` blocks in `term.c`'s input loop:
...
| `capture_menu`
| auto / connected
| `CaptureMenu.run()` — Wren replacement for the historical
  uifc-driven `capture_control` dialog (capture type / save path /
  pause / resume / close).  Driven by Alt+C in `keys_default` and
  the "Capture Control" entry in `online_menu`.

| `music_menu`
| auto / connected
| `MusicMenu.run()` — Wren replacement for the historical uifc-driven
  `music_control` dialog (ANSI music mode picker).  Driven by Alt+M
  in `keys_default` and the "ANSI Music Control" entry in
  `online_menu`.

| `online_menu`
| auto / connected
| `OnlineMenu.run()` — Wren replacement for the historical
  uifc-driven `syncmenu` (the SyncTERM "Online Menu" reached via
...
| Insert a blank custom font at `index` and return its handle.  Names
  ...
  invalid name, allocation failure, safe mode, or when all 210 custom
