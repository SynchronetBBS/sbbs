...
| `online_menu`
| auto / connected
| `OnlineMenu.run()` — Wren implementation of `syncmenu` (the
  SyncTERM "Online Menu" reached via
...
| The persistent Wren main-menu controller.  It loads the menu BBS
  ...
  the configured UIFC palette, and the selected entry's comment and
  ...
  offline form owns the UIFC light-shade application background and
  title/time row;
...
imports it.  Its top level obtains the host-created `Cache` and
`Download` objects.  The auto-load embeds run when their owning VM is
created: once per application for `menu`, and once per BBS session for
`connected`.
...
. `scripts/auto/<event>/<name>.wren` — where `<event>` is `menu` or
  `connected`; this catches imports of auto-load modules which have
  not run yet.
...
model differently, but it is responsible for preserving this user-visible
behavior.
...
directory help, and operation-specific alerts and confirmations retain the
...
entry; a copied read-only entry retains its name when pasted.

The specialized editors preserve the direct menu operations.  Enter on a
sort profile opens its field list, and Enter on a sort field reverses that
...
four cell-size paths, and selecting a path opens the native picker directly.
...
and uses `%` to restore all three components from the mode default.  Enter
commits the current component; Tab and Backtab move without accepting
uncommitted text.

Safe mode retains accepted default-entry and web-list changes in memory while
suppressing their configuration-file writes.  Encryption conversions are
no-ops; Change Password still presents its password prompt before returning.

Repeating editors retain their highlighted row after an action rebuilds the
list.  This applies to entry logging and palettes, sort profiles and fields,
program settings and their nested lists, web lists, and font management.
...
profile visited.
New entries are sorted again after their connection details are accepted.
Quick Connect retains the directory address field's 64-character input limit.
Current Screen Mode retains its original distinction from the Startup Screen
Mode setting in both its dialog title and context help.
Directory password and system-password rows remain obscured in the editor
list, but selecting one uses ordinary text editing so the existing value can
be inspected and corrected.  Personal-directory encryption prompts remain
masked.
Rate choices select the first supported rate at or above a configured value,
or Current when no supported rate is high enough, matching the directory
editor's established catalog behavior.
...
Escape and pane close buttons still only leave a connected Alt+E menu.
...
Accepted UIFC colour changes rebuild the active Wren menu theme immediately.
...
but otherwise preserves the entered name and the file returned by the picker
without adding validation that the native menu did not perform.  It reports a
full custom-font table before prompting for another name.
The preformatted File Locations and Build Options viewers retain the original
titles, label emphasis, indentation, dimensions, and section layout while
using the shared Markdown-capable Help renderer.
Palette Insert and Delete are exposed only while another color can be added or
removed, and sort-profile Paste is exposed only after Copy or Cut has supplied
a clipboard profile.
Insert, Delete, and the blank add row remain available from Font Details as
well as the outer Font Management list.
Adding a web list skips its initial fetch when no cache path is available but
still records the configuration; explicit refresh continues to report the
missing cache.
...
commits the accepted edit before focus moves.  The entry editor keeps its
highlighted field while `[` and `]` move between entries.  Program Settings
again summarizes enabled audio backends, and entry rows distinguish automatic
...
Saving an existing directory entry preserves its `Added` timestamp.  Clearing
the Terminal Type field writes the empty override so a subsequent directory
reload continues to use automatic terminal-type selection.
In the palette component editor, `%` restores all three components of the
selected colour from the mode palette while retaining the current field.
Creating or copying a personal entry with the same name as a read-only entry
replaces that row immediately.  The model retains the shadowed entry so it
reappears if the personal override is deleted, or under its original name if
the personal entry is renamed.
...
directory pane has focus, its selection supplies the OS window title; the
settings pane restores the bare SyncTERM version.  Editing a comment leaves
the title from the pane where editing began until focus moves again.
...
<<ui-help,Help viewer>>; the removed UIFC `helpbuf` markup is not parsed.
...
entry or Program Settings editor closes.  Font changes remain in the font
...
Escape from an owning editor closes it and retains the edits already made.

=== VM Lifetimes and Trust Boundaries [[vm-trust-boundaries]]

SyncTERM deliberately uses two Wren VMs.  They do not share Wren
...
* The *menu VM* is created once after conio and configuration are
  ...
  invoke the trusted native file picker.
...
At every ownership transition (connect, disconnect, and entry to or
return from the menu through Alt+E), the host establishes a new input
epoch.  It discards pending conio keyboard and mouse pushback, clears
input claims and waiters owned by the screen being left, and rejects
late input-shaped results tagged with an older epoch.  This prevents a
connected script from placing synthetic input in `ungetkey()` /
`ungetmouse()` just before the trusted menu or picker takes control.
The same barrier protects the connected VM when control returns from a
trusted UI.

The native file picker remains a trusted consent broker.  The menu VM
does not receive arbitrary pathname-open authority: picker upload
results are read-only, save results are one-shot write-only grants, and
rights remain attached to the foreign `File` handle as documented in
...
| Paste from the system clipboard onto the wire — codepage-aware,
  bracketed-paste-aware.  No-op when the clipboard is empty.  Wraps
  the historical Shift+Insert handler.
...
| Invoke `ScrollbackView.run()` in the connected VM.  The Wren view
  ...
  terminal and mouse subscriptions on exit.  This is the default Alt+B
  path.

| `Conn.upload()`
| Open the upload picker / dialog (equivalent to the legacy Alt+U
  path).  No-op outside an active session.  Mouse events are
  refreshed on the way out.

| `Conn.download()`
| Open the download picker / dialog (equivalent to the legacy Alt+D
  path).  No-op outside an active session.  Mouse events are
  refreshed on the way out.
...
| Load all lists and replace the current model only after the complete
  ...
  `MenuReadStatus` code.  A failed password, decrypt, parse, or
  migration leaves the preceding model and its generation intact.
...
setters change only that snapshot and set `dirty`.  `apply()` validates
the complete snapshot and applies it to the running program without
writing `syncterm.ini`.  `reload()` discards changes not yet applied and
repopulates the same handle from the current running settings.
...
`save()` validates the complete snapshot, preserves unrelated INI
sections, writes the existing `syncterm.ini` keys, and applies the
settings which have immediate effects.  These include scrollback
capacity, scaling, mouse-wheel direction, audio backends, custom-mode
dimensions, cursor style, and the UIFC colors still used by the trusted
native picker and shipped Wren main menu.  It returns `false` when
validation, allocation, encrypted-list rewriting, or INI persistence
fails.  An unapplied snapshot is not applied after such a failure;
settings already accepted through `apply()` remain active.  Changing
the KDF work factor may rewrite an encrypted BBS list during `apply()`.
Saving `syncterm.ini` is disabled in safe mode.
...
The editor keeps UIFC Colours and Custom Screen Mode as nested groups instead
...
| UIFC palette indexes used by the shipped Wren main menu and trusted
  native picker.  Frame, text, and lightbar foreground use `Menu.colors`,
  ...
  The shipped `main_menu.wren` resolves those sentinels to the configured
  default colors and builds an App `Theme`; a custom main-menu override must
  apply the values itself if it wants to preserve that preference.
|===
...
| Returns a fresh `Directory` rooted at the BBS's configured
  ...
  for old configs), or doesn't exist as a directory on disk —
...
| `Host.pickFile(initialDir, mask, opts, title)`
...
| Multi-select counterpart of `pickFile`.  Same argument shape.
  ...
  and has a `.token`), or `null` on cancel / empty selection.  Per
...
| Highest valid OOII mode value (returns `0` in
  ...
  increment back to `0` when it exceeds this cap, matching the
  historical SM_OOII cycle.
...
should be shown while focused; and `cursorShape` optionally returns
`"normal"` or `"solid"`.  `App` reads all three off the focused leaf
each frame and applies them.  A visible null shape defaults to
`"normal"`.  Containers forward the focused child's values.
...
side: the UIFC convention that text never touches the frame.
...
(matches UIFC's `ulist`); each wheel notch moves the selection one
row using the same wrapping rules as Up and Down.  The scrollbar's
top and bottom arrow cells invoke PageUp and PageDown; track clicks
are ignored.  Button-1 drags fall through to screen selection.
...
Keys follow UIFC's line editor.  `Left`, `Right`, `Home`, `End`,
...
field.  `cursorShape` is `"solid"` in insert mode and `"normal"` in overwrite
mode, matching UIFC's visible mode indicator.  Pasted control characters are
discarded, matching typed-input filtering, and `maxLen` applies to pasted
text.
...
The read-only `input` getter exposes that `TextInput` so trusted UI
code can set `maxLen` or a display-only `mask` before presenting the
popup.
...
padding, no buttons, no `[X]`).  Total height is 3 cells.  Used by
`ListView`'s Ctrl-F.
...
All popups use UIFC-style frame interior padding via
...
any other key dismisses.  Wheel scrolls one line; scrollbar
clicks/drags jump proportionally.  An ordinary mouse-button click also
dismisses, including a click outside the dialog.  Same scrollbar knobs as
`ListView` (`scrollbarSide=`, `scrollbarSeparator=`).  Clicking the
frame's `[X]` corner button dismisses (inherited from `Pane`).
...
where text is flush.  The scrollbar / separator span the full
inner height (between the top and bottom frame chars); only text
rows inset by the top/bottom padding.
...
text with one styled space cell on each end (`" Title "`), matching
UIFC's backtick-marked title convention.
