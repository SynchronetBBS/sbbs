...
import "ui" for App, Pane, ListView, TextInput, Button,
               Alert, Confirm, Prompt, Help, PopStatus,
...
* `popStatus(message)` — show or clear a transient centered overlay
  ...
  while blocking on `Input.next` or a long Wren computation.
...
auto-suffixes `.inactive` whenever `inActiveLayer` is false (the
widget is part of a pane behind a modal), so a single layer-state
flag cascades through frame, title, list rows, input field, button
labels, and scrollbar without per-widget bookkeeping.
...
an ASCII-safe substitute.  Set `theme.glyphs.asciiOnly = true` to
force the fallback everywhere (e.g. on emulations where the rich
codepoint can't be represented).
