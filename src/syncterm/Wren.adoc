...
and uses `%` to restore all three components from the mode default.
...
Add, update, delete, and save are disabled in safe mode before any fetch,
in-memory mutation, or write.  Refresh remains a read/fetch operation and
is permitted.  No web-list method exposes its cache pathname as a file
capability.
