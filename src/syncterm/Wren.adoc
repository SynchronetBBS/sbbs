...
| `true` consumes the event (no later hook sees it; SyncTERM's
  ...
  `onInput` byte replacement, `onStatus` text replacement).  Any
  other return — `false`, `null`, a number, an instance, … — is a
  passthrough: the next hook in the chain sees the unmodified event,
  and SyncTERM's default handling proceeds.
...
| C1 (only the `String`-replace branch is meaningful; the status bar
  has no notion of "consume")
...
| `Host.uploadArrow=(b)`, `Host.downloadArrow=(b)`
| Status-bar transfer-indicator arrows (CP437 ↑ / ↓ at columns
  28-29).  Generic — any Wren script that knows it's transferring
  something can light them; the canonical user is the SFTP queue's
  worker fibers, but a Zmodem / HTTP fetcher / etc. could too.
  Both default to `false` and are reset on session teardown.
  Setter only — there's no getter, the host owns the rendered
  state.
