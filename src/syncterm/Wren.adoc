...
| Default `Hook.onKey` bindings for the in-terminal Alt-shortcuts
  ...
  loop: Alt+O (toggle mouse-event reporting) and Alt+Up / Alt+Down
...
| `emulation` (`Emulation`), `doorwayMode`, `music`, `started`,
  ...
  (`StatusDisplay`), `atasciiInverse` (Bool — ATASCII inverse-video
  mode), `ooiiMode` (0 = off, 1..3 = OOTerm / OOTerm1 / OOTerm2)
