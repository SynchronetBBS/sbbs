...
Ctrl-S, `<`/`>`, Ctrl-D, and Alt-B invoke their documented commands.  The
...
sixteen colors are present.  Delete and the editor-specific copy, paste,
rename, and cut keys are direct list commands.
...
and uses `%` to restore the selected component's mode default.
...
Sort-profile and web-list name prompts repeat after a duplicate name, and
sort-profile field navigation returns to the last profile visited.
