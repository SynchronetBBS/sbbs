...
is being edited, the footer retains the hints for the pane that opened it.
...
the title from the pane where editing began until focus moves again.  The
active editor is left-aligned within the middle `screen width - 4` cells and
uses the normal text-input lightbar and cursor.  Leaving the editor restores
the centered inactive row immediately, even when its value was unchanged.
...
An owning menu remains on the modal stack and is rendered behind a nested
...
backends, custom-mode dimensions, cursor style, and the UIFC colors still
used by the trusted native picker and shipped Wren main menu.  It returns
