...
write remains grouped at the historical screen-exit boundary.
...
| Stored personal-list path or URI.  The host resolves it to the active
  local or cached list path after a successful apply or save.
...
| Positive scrollback capacity and modem baud rate.
...
| UIFC palette indexes used by the shipped Wren main menu and trusted
  native picker.  Foreground catalogs include the `Default` entry at index
  16; background catalogs include it at index 8.  The shipped
  `main_menu.wren` resolves those sentinels to the configured default colors and
  builds an App `Theme`; a custom main-menu override must apply the values
  itself if it wants to preserve that preference.
|===
