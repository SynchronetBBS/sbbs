...
(matches UIFC's `ulist`); wheel scrolls three rows; clicks on the
...
`insertMode` reports whether printable input inserts (`true`, the default) or
overwrites existing characters (`false`).  `cursorShape` is `"solid"` in
insert mode and `"normal"` in overwrite mode, matching UIFC's visible mode
indicator.  Pasted control characters are discarded, matching typed-input
filtering, and `maxLen` applies to pasted text.
