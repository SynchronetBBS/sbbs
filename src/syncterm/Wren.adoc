...
and uses `%` to restore all three components from the mode default.  Each
...
`SelectOnFocusInput` is the existing-value editing variant used by UIFC-style
...
initial value starts selected, matching UIFC `K_EDIT`: typing, keyboard paste,
...
`mask` before presenting the popup.
