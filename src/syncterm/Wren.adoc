...
SyncTERM embeds http://wren.io/[Wren] in two isolated scripting hosts.
A persistent trusted VM owns the main menu, while every connection gets
a fresh restricted VM which is torn down at disconnect.  Connected
scripts hook into keyboard, mouse, inbound bytes, outbound bytes,
status text, and a periodic timer; menu scripts implement the dialing
directory and settings UI through a separate set of capabilities.
...
replaced with
`~/.local/share/syncterm/scripts/wren_console.wren`; both VMs import
that library when their built-in menu or connected entry point opens
...
<<ui-help,Help viewer>>.  Help content must be Markdown; UIFC `helpbuf` markup
is unsupported.
...
`MenuUi.prompt` wrappers use the field's `maxLen` for this width, matching UIFC
input sizing: short fields retain the normal minimum while URI, path, password,
