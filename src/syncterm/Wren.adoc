...
| Ignored — `onMatch` is passthrough-only.  Use `Hook.onInput` (per-
  byte, drop / replace) when you need to remove bytes from the wire.
...
| Fires once when the SSH shell channel closes but the session is
  still alive (e.g. SFTP transfers may still be in flight).
  No argument; return value ignored.  Typical use: pop a
  status / queue UI so the user can watch transfers finish, or
  signal a worker to wind down.  See <<cterm,CTerm.sftpActive>>
  for the corresponding "keep the session up" flag.
| (none — return value ignored)

| `Hook.onDisconnect(fn)`
| Fires at the end of the disconnect path, after the main loop
  has fully exited but before the BBS / Wren VM tear down.  No
  argument; return value ignored.  SFTP is no longer available
  at this point (sftpc_finish has already run); use this for
  final local-state flushes (queue persistence, log close, etc.).
| (none — return value ignored)
|===
...
the `Cache` / `Download` / `Upload` module-level vars (see below)
are pre-bound from these at module load.
...
| `Host.uploadDir`
| Same shape as `downloadDir` but for `UploadPath`.  Used as the
  module-level `Upload` binding.
...
| Re-construct a `File` foreign from a consent token previously
  ...
  for a path outside the sandboxed `Cache` / `Upload` /
  `Download` roots; the token *is* the consent.
...
`Download`, `Upload`, and `Cache.list[someSubdir]` (and the
analogous `Download.list[...]` / `Upload.list[...]`) are the
only paths to one.

`Download` and `Upload` are module-level `Directory` bindings
pre-bound from `Host.downloadDir` / `Host.uploadDir` at import
time.  Both are `null` when the corresponding BBS path is empty
or set to the user's `$HOME`; scripts MUST null-check before use.
Their relaxed-name predicate is what makes typical SFTP filenames
("file with spaces.zip", "doc-v1.0.tar.gz") usable as `local`
paths in transfer queues.
...
| Opaque consent token for picker-sourced `File`s; `null` for
  `File`s obtained via `Cache` / `Upload` / `Download` Directory
  listings.  Persist this in WON next to the path string and pass
...
paths — that's the whole point of the `Cache` / `Upload` /
`Download` sandboxed roots and the relaxed-name predicate.  The
escape hatch is `Host.pickFile` / `Host.pickFiles`, where the UIFC
...
This shape is *only* safe in a fiber whose only resume sources are
its own SFTP completions.  If the calling fiber has other things
that can wake it (`Wake.post`, `Timer.trigger`, the App's claim
handler posting events to its run-fiber, etc.), the yield can
return a value of the wrong type and the `r is SFTPError` check
will see something other than a String.  From inside an `App`
handler, wrap the SFTP call with `<<ui-app,App.runChild>>`, which
spawns a dedicated child fiber and pumps the App's event loop
while it's parked:
...
| Parse `text` and return the reconstructed value.  Aborts the fiber with
  a message containing the byte offset on syntax errors, premature end of
  input, trailing garbage, or a `List` / `Map` used as a `Map` key.
  Tolerates arbitrary whitespace and trailing commas inside `[...]` /
  `{...}`.  Accepts exactly the string-escape set the Wren compiler
  itself recognises.
|===
...
A keymap or widget handler that needs to do an SFTP op (or any
other yielding work) cannot just write `SFTP.foo(Fiber.current,
...) || Fiber.yield()` and expect it to work — the App's run-
fiber has multiple resume sources (the claim handler's `Wake.post`
for keys/mouse, the `tickMs` timer, external `app.post(value)`
calls), and any of them might wake the fiber before the SFTP
result arrives.  The yield then returns the wrong type and the
calling code sees garbage.

`runChild(fn)` solves this by spawning a child fiber whose only
resume sources are its own SFTP / `Wake.post` completions:
